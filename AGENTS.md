...
- Chinese name: `投稿舱`
- Current prerelease display version: `V0.15`
- Category: local-first manuscript submission workspace
- Desktop stack: Tauri 2.x, React 18, TypeScript 5, Vite 5, Rust/Cargo

## Versioning

- Show the prerelease version immediately after the product name on primary identity surfaces: `投稿舱 ManuscriptDock V0.15`.
- After each user-visible product update, increment the display version by `0.01` using two digits after the decimal (`V0.15` → `V0.16`; `V0.19` → `V0.20`).
- Keep package and installer metadata SemVer-compatible (`V0.15` maps to `0.15.0`).
