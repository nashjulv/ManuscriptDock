...
- Chinese name: `投稿舱`
- Current prerelease display version: `V0.61`
...
- Show the prerelease version immediately after the product name on primary identity surfaces: `投稿舱 ManuscriptDock V0.61`.
...
- Keep package and installer metadata SemVer-compatible (`V0.61` maps to `0.61.0`).
...
- Optional AI assistance is authorized for cover-letter/highlights drafting, single-material review, and cross-material consistency review. Rust owns bounded network requests, system credentials, per-request source preview/consent, and audit records. AI results never confirm author facts or overwrite existing materials. Knowledge-body/PWC flows remain outside scope. Preserve existing user data and historical transmission records. See `docs/optional-ai-assistance.md`.
...
- The primary product identity intentionally remains bilingual as `投稿舱 ManuscriptDock V0.61`; do not remove either name as an internationalization cleanup.
