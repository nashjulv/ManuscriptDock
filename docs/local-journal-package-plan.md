...
- 工作分支：`feature/submission-assistant`
...
基础任务无需外部论文模型。V0.60 按用户授权增加[可选 AI 材料助手](optional-ai-assistance.md)，限定起草与检查范围，每次预览资料并同意才可调用。知识体问答、PWC 评审与发布仍不恢复。保留旧数据及历史外发记录，不能把过去外发过的稿件显示成“从未离开设备”。新功能使用独立的系统凭据条目，不修改历史凭据。
...
默认展示“本地优先”，AI 预览明确区分本机与外部服务；基础功能无需配置模型，原稿始终保留。不以固定“0 KB”徽标替代测量，也不声称能控制用户选择目录上的其他同步软件。单项目删除只处理应用管理的数据，原始文件和已另存输出仍由作者管理。
