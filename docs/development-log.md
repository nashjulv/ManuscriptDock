...
同页缺项、跨页落点、材料待办、检查核验和文章类型修改统一使用黄色高亮，每秒一次、持续 4 秒后自动清除；再次点击重启，输入或勾选提前结束。提示文字与键盘焦点保留，系统减少动态效果时改为静态黄色并按时消失。共享清理逻辑避免旧定时器清除后来触发的高亮。

验证：前端 141 项通过，构建与差异检查通过；新增中英文定时消失、重复触发、跨页落点及输入清除测试。Internationalization review：zh-CN/en 的提示可读性和交互均检查，本次无新增界面文案或翻译键。

浏览器验证：Browser plugin not available，使用已有 Playwright 与 Chrome，在 `127.0.0.1:1424` 的中英文合成模型设置中确认黄色色值、透明闪烁阶段、1 秒 × 4 次动画、到期移除、提示保留及减少动态效果模式。无页面或控制台错误，截图和结果位于 `/tmp/md-highlight-evidence/`。未打包安装、提交或推送。

## 2026-09-08 · 移除类型勾选与核验返工修复

移除文章类型强制勾选，保留推荐类型和修改入口。补充条件适用性核验、结构无变化保存及按证据范围失效、旧要求摘录兼容、检查页就地核验和保存后本地复查、存在性要求自动检查、重复摘要长度核验合并、直接返回导航。详细行为见[使用手册](user-manual.md)和[后续修正说明](action-prerequisite-guidance-audit.md)。沿用已有工作区版本元数据，本次未再次调整版本。

验证：前端 137 项通过；Rust 全工作区 161 项通过、2 项既有忽略；TypeScript/Vite 构建、格式和差异检查通过。Clippy 全工作区全目标通过。额外补测带“所有稿件”措辞但仍有条件限定的适用性，避免误判成无条件要求。Browser plugin not available，使用已有 Playwright 与系统 Chrome，在 `127.0.0.1:1424` 的中英文合成 Tauri 工作区验证直接设目标、修改类型定位、无额外弹框的返回、保留上传前置引导，以及检查页核验后完成状态与待确认数更新。检查 1440×1000 与 1024×768，页面非空、无框架错误覆盖层、无相关控制台错误。

临时截图和结果位于 `/tmp/md-friction-evidence/`、`/tmp/md-friction-check-evidence/`。后端覆盖真实合成文件的检查、核验、导出和重开；未重新运行用户真实 PDF 的原生完整流程，未打包、安装、提交或推送。

Internationalization review：zh-CN/en 的类型修改、核验成功/失败、不适用依据、旧记录说明、自动检查文案和动态完成状态均已覆盖。原稿与官方引用保持原文；旧规则、模型设置和联网授权限制保留。

## 2026-09-08 · 操作前置条件统一引导

按用户批准的 22 组场景接入 `GuidedButton`：条件不满足时保留按钮点击，本页展开、滚动、聚焦和高亮缺项，跨页先说明再导航。补齐上传空清单入口、材料纳入条件说明、知识体数量限制说明，保留执行中锁定和逐次人工授权。上传、要求与导出在点击时重查快照有效性，跨页定位等待异步内容，问答问题草稿在工作区内保留。详见[实施与验证](action-prerequisite-guidance-audit.md)。

验证：全量前端 131 项通过，TypeScript / Vite 生产构建与差异检查通过；中英文各完成合成工作区的上传缺项弹框、跨页跳转、文章类型聚焦和模型密钥聚焦；1440×1000 与 1024×768 视口检查通过，无相关控制台错误。此前布局任务记录的 21 项旧置灰断言已随新交互更新并通过。本次未重新验证原生 WebView、打包安装或操作真实稿件。并行布局任务已将版本改为 V0.52，本次未再次调整版本。

Internationalization review：zh-CN/en 新增提示、弹框、动态原因、键盘操作、缺项与失败路径已检查；稿件及引用原文保持原语言。

## 2026-09-08 · V0.52 工作区卡片间距

四步投稿准备卡片增加 18px 顶部留白，左右与页头、正文卡片和检查/修订导航共用自适应间距（24–58px，620px 以下为 16px）。卡片间距统一为 12px，内部留白为 14px / 16px；修正窄屏字号样式覆盖正文留白的问题。为检查/修订导航保留独立网格行，正文卡片区域使用剩余高度并独立滚动。

验证：新增 zh-CN / en 工作区导航和流程状态回归测试；布局与字号定向测试 18 项通过。使用合成工作区、模拟 Tauri 返回值，在本地 Vite 页面通过 Playwright + Chrome 检查桌面与窄屏、默认/大字号、卡片边距、检查页和依据面板切换；页面非空、无错误覆盖层、无控制台错误。构建通过。当前工作树全量测试另有 21 项失败（80 项通过，新增布局测试前），主要涉及按钮禁用状态断言；本次不修改这些功能行为。未重新打包或验证原生 WebView。

Internationalization review：检查 zh-CN / en 的流程标题、说明、进度状态及导航；未新增文案或更改稿件内容，窄屏保持既有两列卡片和简化说明规则。版本与安装元数据同步为 V0.52 / 0.52.0。

## 2026-09-08 · V0.51 单次官网读取授权提示

读取中保留主授权和额外域名的勾选并禁用，显示“本次已授权，正在读取”；成功、部分完成、失败后统一清空并提示重新授权。防止重复点击发起多个请求，旧请求返回时不会清空新目标的授权；读取中隐藏上次结果，避免旧的完成提示与当前读取状态混淆。版本保持 V0.51 / 0.51.0。

验证：OfficialSourceAccess 与 App 共 40 项测试通过，覆盖中英文读取中、成功、部分完成、两种失败返回、额外域名、重复点击、跨目标迟到结果；TypeScript / Vite 构建通过。Internationalization review：新增处理中与授权结束提示均提供 `zh-CN` / `en`，失败复用已本地化错误码，没有改变后端授权范围。

macOS 隔离原生应用实际读取公开期刊页面，确认读取中勾选保留且禁用、显示授权进行中；成功读取并保存 5 项要求后，勾选清空、获取按钮禁用、显示授权结束。核对中英文提示并保存 `consent-*` 截图与 AX 记录。未上传论文、未替换日常安装；失败和跨目标状态由定向自动测试覆盖。

## 2026-09-08 · V0.51 已有档案列表折叠

按用户要求保持 V0.51 / 0.51.0。导入时匹配的已有本地档案默认最多展示 3 个，超过时提供含剩余数量的展开按钮和收起按钮；切换所选稿件时恢复收起。保留逐档案继续入口，补充中英文与键盘操作测试。

验证：相关组件与 App 共 21 项测试通过，TypeScript / Vite 构建通过，`git diff --check` 通过。macOS 隔离原生应用用 5 个真实创建的合成档案验证默认 3 项、展开 5 项、收起 3 项，中英文按钮与剩余数量正确。Internationalization review：检查 `zh-CN` / `en` 标题、按钮、动态数量及键盘可访问性，无新增翻译缺口。仅本地修改与隔离验证，未替换日常安装。

## 2026-09-08 · V0.51 投稿包导出状态

导出记录落入本地工作区并绑定当前核验上下文，侧栏、概览和投稿包页面统一显示可导出、已导出、内容已变更及历史待核验状态。投稿登记独立于导出完成；取消、失败和目录丢失保留历史事实。版本同步为 V0.51 / 0.51.0，详见[修复与验证记录](package-export-status.md)。

## 2026-09-08 · V0.50 主流程修复

本轮实现状态一致、识别核对、官方要求去重与四种决定、基础检查解耦、预检导出和来源恢复。完整验证、真实 PDF 对照及尚未关闭的 R6/平台范围见[执行记录](first-use-flow-v050-validation.md)。以下既有版本条目与测试数量属于历史记录，不作为本轮验证证明。
