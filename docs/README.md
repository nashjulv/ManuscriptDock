# 投稿舱 ManuscriptDock V0.60 文档

## 当前方向与实施入口

V0.60 在本地核心之外增加[可选 AI 材料助手](optional-ai-assistance.md)：投稿信与亮点起草、材料语义及一致性检查。逐次预览资料并确认，AI 不替代作者确认，现有文件不会被覆盖。此扩展更新早期方案中“不提供外部模型设置”的范围约束。验证边界见 [V0.60 实现记录](releases/V0.60/implementation-status.md)。

2026-09-14 确认收敛为“推荐期刊”和“按目标期刊整理投稿包”。V0.54 建立 PDF／DOCX 本地入口、可恢复最近任务移除与受控 IPC；V0.55—V0.60 的目录、材料状态和 AI 增量以 [V0.60 实现状态](releases/V0.60/implementation-status.md)为准。规则由团队人工核对后直接维护仓库配置并内置客户端，首版不开发维护功能或独立更新服务。

进一步确认：实际实现可重写全部旧代码与交互，删除无用功能，不以旧体系兼容为目标；由于早期版本尚未上线，旧任务入口、扫描、导入和迁移运行时均已移除，应用不会主动删除磁盘上已有文件。当前详细方案集中在 `implementation/`，product/reference/archive 目录仍按迁移计划后续整理；历史核心验证保留在 [V0.54 记录](releases/V0.54/implementation-status.md)，当前增量验证见 V0.60 记录。
...
- [当前使用指南](current-user-guide.md)：V0.60 双任务、目录操作、材料状态、AI 辅助与导出，中英文操作摘要。
- [历史产品使用手册](user-manual.md)：V0.52 旧界面历史手册，不作为当前操作指南。
