...
单一学术知识体不是“一篇论文的摘要”，而是围绕一个或一组 `Claim` 构成的、可追溯、
可版本化且能够声明服务能力的知识单元。单体采用身份与版本、知识边界与证据、能力契约、
交互与执行运行时、验证权利与信誉五部分架构；以下对象归入“知识、边界与证据”层：
...
能力由版本化 `CapabilityContract` 显式声明，模型只是可替换运行时。内容快照不可变，
ReputationRecord 则在稳定知识体身份下独立演进。完整单体协议和空间映射见
...
单一知识体固化时，首个 `ClassificationAssignment` 由作者从内置双语学科索引中确认，
并进入知识体记录哈希。分类不是从论文标题推测出的事实。后续模型可以提交候选代码、置信
说明和来源依据，但只有作者确认后的版本能够替代当前分类并生成新的知识体哈希。
...
MVP 的 Rust 可信核心已经定义 schema v2 本地快照、五部分服务架构、十类论文核心对象、
版本化能力契约、可替换 RuntimeProfile、独立 ReputationRecord 引用、Claim 内部语义
约束、独立 `AIReviewReport` 版本链、八类关系协议和网络声明校验。当前真实工作区只注册
本地单体、不可变稿件来源和建构状态，不会虚构外部研究、AI 审核或跨体关系。
