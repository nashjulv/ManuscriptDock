...
export const OFFICIAL_SOURCE_MESSAGES: Record<string, [string, string]> = {
  ...
  OFFICIAL_SOURCE_UNAVAILABLE: ["暂时无法访问期刊网站，请稍后重试或粘贴官方原文。", "The journal website is currently unreachable. Retry later or paste official text."],
  ...
  OFFICIAL_CONNECTION_FAILED: ["无法连接期刊网站，请稍后重试或粘贴官方原文。", "The journal website could not be reached. Retry later or paste official text."],
  ...
  OFFICIAL_HTTP_CONFIRMATION: ["旧记录：当时因 HTTP 确认而暂停；现在重新获取即可自动处理。", "Historical record: HTTP confirmation paused this request. Capture again to handle it automatically."],
  OFFICIAL_HTTP_USED: ["本次通过未加密 HTTP 读取公开页面", "Reading a public page over unencrypted HTTP"],
  OFFICIAL_VIRTUAL_DNS: ["检测到虚拟 DNS 地址，正在通过 Cloudflare 加密 DNS 查询真实公网地址。", "Virtual DNS detected; resolving the real public address using Cloudflare encrypted DNS."],
  OFFICIAL_DNS_RECOVERED: ["已通过加密 DNS 取得并校验公网地址，将连接绑定到该解析结果。", "Encrypted DNS returned validated public addresses; the connection is pinned to these results."],
  OFFICIAL_ENCRYPTED_DNS_FAILED: ["虚拟 DNS 兼容解析未成功，请检查网络后重试或粘贴官方原文。", "Encrypted DNS recovery from a virtual address failed. Check the network and retry, or paste official text."],
  OFFICIAL_SOURCE_CORRECTED: ["依据已核验的官方来源修正本次访问地址；历史投稿记录保持原样。", "The access URL was corrected using a verified official source; historical target records are preserved."],
  ...
  OFFICIAL_ENCODING_INFERRED: ["页面未声明编码，依据中文语言标记按 GB18030 解码，请核对原文。", "No charset was declared; the Chinese language hint was used to decode GB18030. Review the source text."],
  OFFICIAL_GZIP_DECODED: ["已解压网页正文，解压后仍受 2 MB 上限约束。", "Page text decompressed; the 2 MB limit also applies to decoded content."],
  OFFICIAL_COMPRESSION_FAILED: ["网页压缩内容损坏或不完整，请重试或粘贴官方原文。", "The compressed page is damaged or incomplete. Retry or paste official text."],
