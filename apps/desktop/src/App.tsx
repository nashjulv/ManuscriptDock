...
interface InstitutionRuleEvidence { status: "search_required" | "candidate_sources_found" | "verified" | "no_official_rule_found"; ruleSetId: string | null; ruleSetVersion: string | null; sourceUrls: string[]; verifiedAt: string | null; recognizedRankTiers: string[]; blockedRankTiers: string[]; }
...
interface JournalRecommendationRun { schemaVersion: number; runId: string; workspaceId: string; manuscriptVersion: number; manuscriptHash: string; algorithmVersion: string; catalogVersion: string; catalogVerifiedDate: string; inferredTopic: ResearchTopic; topicBasis: string; maturityScore: number; evaluatedUnixMs: number; recommendationProfile: JournalRecommendationProfile; deadlineDaysRemaining: number; preferences: JournalMatchPreferences; domestic: JournalRecommendation[]; international: JournalRecommendation[]; schoolRuleStatus: string; limitations: string[]; externalTransmission: "not_performed"; }
...
function JournalMatchStage({ workspace, onContinue }: { workspace: WorkspaceSummary; onContinue: () => void }) {
  ...
  async function calculate() {
    ...
    try {
      ...
      setRun(await invoke<JournalRecommendationRun>("recommend_journals", { workspaceId: workspace.id, profileId: savedProfile.profileId, preferences }));
    }
  ...
  return <>
    <PanelHeading kicker={text("步骤 5 / 8 · 期刊匹配", "Step 5 / 8 · Journal match")} title={text("先确定投稿背景，再推荐期刊", "Set the submission context before matching")} copy={text("投稿人、学校、专业、论文用途和截止日期共同形成一份本地档案；任何字段变化都会生成新的推荐记录。", "Author, institution, specialty, purpose, and deadline form a local profile. Any change creates a new recommendation run.")} />
    <BoundaryNote title={text("学校标准是重要依据", "Institution rules are material evidence")} copy={text("正式推荐应检索学校官网、研究生院、科研管理部门或学院发布的有效规则。只有带官方来源、适用范围、有效期和核验状态的规则才使用 24% 学校规则权重；当前未核验时只输出候选初筛，不按学校声望推断。联网检索必须另行获得你的授权。", "Formal matching should search valid rules published by the institution, graduate school, research office, or faculty. Only verified rules with official sources, scope, and validity may use the 24% institution-rule weight. Until then results are provisional and never infer institutional prestige. Online discovery requires separate authorization.")} />
...
function EvidencePane({ stage, workspace, structureReport, readinessReport, knowledgeBodySnapshot = null, knowledgeBodyRecord = null, attestation = null, submission = null, submissionExport = null, ruleCatalog = [], selectedRulePackIds = [], submissionElementCatalog = null, revisionDraft = null, revisionValues = {}, revisionResult = null, versionHistory = null, selectedVersion = null, versionComparison = null, isComparingVersions = false }: PaneProps) {
  ...
  if (stage === "journals") return <EvidenceFrame kicker={text("目录与算法证据", "Catalog and algorithm evidence")} title={text("公开来源、本地评分", "Public sources, local scoring")}><div className="document-sheet source-sheet"><span className="document-type">LOCAL FIT V1.1</span><p className="document-title">computer-ai-2025.1</p><dl><div><dt>{text("国内目录", "Domestic directory")}</dt><dd>CCF 2025 · T1/T2/T3</dd></div><div><dt>{text("国际目录", "International directory")}</dt><dd>CCF AI · A/B/C</dd></div><div><dt>{text("评分权重", "Weights")}</dt><dd>{text("校规 24 · 主题 18 · 专业 5 · 类型 8 · 完备度 10 · 语言 6 · 策略 8 · 用途 8 · 时间 10 · 开放获取 3", "Institution 24 · Topic 18 · Specialty 5 · Type 8 · Readiness 10 · Language 6 · Strategy 8 · Purpose 8 · Time 10 · OA 3")}</dd></div><div><dt>{text("学校规则", "Institution rules")}</dt><dd>{text("须检索官方来源并核验；未核验时排除 24% 权重并标记候选初筛", "Official sources must be discovered and verified; until then the 24% weight is excluded and the shortlist is provisional")}</dd></div><div><dt>{text("身份公平", "Identity fairness")}</dt><dd>{text("姓名仅用于归属；学校名称不参与声誉打分", "Name is attribution only; institution prestige is never scored")}</dd></div><div><dt>{text("外部传输", "External transmission")}</dt><dd>{text("未发生", "None")}</dd></div></dl><small>{text("国内与国际目录相互独立；综合分不是录用概率，投稿前须核对学校正式文件和期刊官网。", "Domestic and international directories are independent. The fit score is not an acceptance probability; verify official institution rules and the journal website before submission.")}</small></div></EvidenceFrame>;
