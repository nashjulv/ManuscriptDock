...
function authorMaterialInputsReady(catalog: SubmissionMaterialCatalog | null) {
  ...
  return catalog.checklist
    .filter((item) => item.blocking && item.id !== "target-journal" && item.id !== "current-check" && item.id !== "official-journal-requirements")
...
function SubmissionFlowRail({ catalog }: { catalog: SubmissionMaterialCatalog | null }) {
  ...
  const steps = [
    ...
    { id: "ready", label: text("投稿已就绪", "Submission ready"), copy: text("材料齐备并完成目标检查", "Materials complete and target checks passed"), complete: Boolean(catalog?.targetCheckReady), active: catalog?.workflowStatus === "target_verified" },
  ];
...
const MATERIAL_KIND_OPTIONS: Array<{ kind: SubmissionMaterialKind; zh: string; en: string; copyZh: string; copyEn: string }> = [
  ...
  { kind: "blinded_manuscript", zh: "匿名主稿", en: "Blinded manuscript", copyZh: "移除作者、单位和致谢等身份信息的独立稿件", copyEn: "A separate manuscript with author-identifying information removed" },
...
function submissionChecklistCopy(item: SubmissionMaterialChecklistItem, locale: Locale) {
  ...
  const labels: Record<string, string> = { "main-manuscript": "Current manuscript", "target-journal": "Target journal and article type", "official-journal-requirements": "Official journal requirements", "current-check": "Target-specific recheck", "latex-project": "Complete LaTeX project", "figure-originals": "Original figures" };
  const details: Record<string, string> = {
    ...
    "current-check": item.status === "passed" ? "A target-specific check exists for the current version." : "Run a target-specific check after completing the materials.",
...
function SubmissionMaterialsCenter({ workspace, catalog, busy, confirmingRequirementId, targetReady, onAdd, onConfirm, onContinue }: { workspace: WorkspaceSummary; catalog: SubmissionMaterialCatalog | null; busy: boolean; confirmingRequirementId: string | null; targetReady: boolean; onAdd: (kind: SubmissionMaterialKind) => void; onConfirm: (itemId: string, confirmed: boolean) => void; onContinue: () => void }) {
  const { locale, text } = useI18n();
  ...
  const requiredKinds = new Set((catalog?.checklist ?? []).flatMap((item) => item.materialKind ? [item.materialKind] : item.id.includes("references") ? ["bibliography" as const] : item.group === "declarations" ? ["declaration" as const] : []));
  const visibleOptions = MATERIAL_KIND_OPTIONS.filter((option) => requiredKinds.has(option.kind));
  const authorInputsReady = authorMaterialInputsReady(catalog);
  return <>
    <PanelHeading kicker={text("主任务 3 · 目标专属准备", "Primary task 3 · Target-specific preparation")} title={text("只补充这家期刊真正需要的资料", "Add only what this journal actually requires")} copy={targetReady ? text("清单由当前主稿、文章类型和官方作者指南共同生成；每条篇幅限制和支持文件要求都保留证据。", "The checklist is generated from the current manuscript, article type, and official author guide; every length limit and supporting-file requirement keeps its evidence.") : text("请先选择主投期刊并取得有效的官方要求，再开始补充资料。", "Choose a primary journal and capture valid official requirements before adding materials.")} />
    <section className="materials-checklist" aria-labelledby="materials-checklist-heading"><header><div><span>{text("动态投稿清单", "Dynamic submission checklist")}</span><h3 id="materials-checklist-heading">{authorInputsReady ? text("作者侧必需资料与确认已完成", "Author-supplied files and confirmations are complete") : text("仍有必需项待处理", "Required items still need attention")}</h3></div><strong>{catalog ? `${catalog.requiredCompleted ?? 0}/${catalog.requiredTotal ?? 0}` : `v${workspace.snapshotVersion}`}</strong></header><div className="checklist-groups">{groups.map((group) => { const items = (catalog?.checklist ?? []).filter((item) => item.group === group); if (items.length === 0) return null; return <section className="checklist-group" key={group} aria-labelledby={`checklist-group-${group}`}><header><h4 id={`checklist-group-${group}`}>{localize(locale, ...groupLabels[group])}</h4><span>{items.length}</span></header><ul>{items.map((item) => { const copy = submissionChecklistCopy(item, locale); const confirmed = item.confirmable && item.status === "passed"; return <li key={item.id} data-status={item.status}><Icon name={item.status === "passed" ? "check" : "warning"} /><div><div className="checklist-item-heading"><strong>{copy.label}</strong><span>{submissionChecklistStatusLabel(item.status, locale)}</span></div><p>{copy.detail}</p>{item.evidenceExcerpt ? <details className="checklist-evidence"><summary>{text("查看官方依据", "View official evidence")}</summary><blockquote>{item.evidenceExcerpt}</blockquote><a href={item.sourceUrl ?? undefined} target="_blank" rel="noreferrer">{item.sourceUrl}</a>{item.capturedUnixMs ? <small>{text("获取于", "Captured")} {formatModifiedDate(item.capturedUnixMs, locale)} · {item.freshUntilUnixMs && item.freshUntilUnixMs >= Date.now() ? text("有效", "Current") : text("需更新", "Refresh needed")}</small> : null}</details> : null}{item.confirmable ? <button className="text-button checklist-confirm" type="button" disabled={confirmingRequirementId !== null} onClick={() => onConfirm(item.id, !confirmed)}>{confirmingRequirementId === item.id ? text("正在保存…", "Saving…") : confirmed ? text("撤销确认", "Revoke confirmation") : item.verification === "author" ? text("我已确认内容真实且适用", "I confirm this is accurate and applicable") : text("我已对照原文人工核验", "I verified this against the source")}</button> : null}</div><span>{item.blocking ? text("必需", "Required") : text("建议", "Recommended")}</span></li>; })}</ul></section>; })}</div></section>
    {visibleOptions.length > 0 ? <><h3 className="material-upload-heading">{text("需要补充的支持文件", "Supporting files to add")}</h3><div className="material-type-grid">{visibleOptions.map((option) => { const count = catalog?.materials.filter((material) => material.kind === option.kind).length ?? 0; return <article key={option.kind}><div><Icon name={option.kind === "figure" ? "file" : "folder"} /><span>{count}</span></div><h3>{localize(locale, option.zh, option.en)}</h3><p>{localize(locale, option.copyZh, option.copyEn)}</p><button className="secondary-button" type="button" disabled={busy || !targetReady} onClick={() => onAdd(option.kind)}>{busy ? text("正在添加…", "Adding…") : text("添加文件", "Add files")}</button></article>; })}</div></> : null}
    ...
    <BoundaryNote title={text("AI 语义与语法审计 · 后续迭代", "AI semantic and grammar audit · Planned")} copy={text("当前版本不执行语法润色或语义改写，只处理可追溯的投稿准备、格式、篇幅、声明和支持文件要求。AI 审计能力将在后续版本上线，敬请期待。", "This version does not perform grammar polishing or semantic rewriting. It focuses on traceable submission preparation, format, length, declarations, and supporting files. AI audit is planned for a later release.")} />
    <button className="primary-button" type="button" disabled={!targetReady || !authorInputsReady} onClick={onContinue}>{!targetReady ? text("先核验目标期刊", "Verify the target journal first") : authorInputsReady ? text("按目标期刊重新检查", "Run target-specific checks") : text("先补齐清单中的必需项", "Complete the required checklist first")}<Icon name="arrow" /></button>
  </>;
}

function JournalMatchStage({ workspace, selectedTarget, targetPlan, requirementSnapshots, selectingTarget, targetPlanBusyId, requirementBusyId, onRecommendationGenerated, onSelectTarget, onAddBackup, onPromoteBackup, onDiscoverRequirements, onSaveManualRequirements, onContinue }: { workspace: WorkspaceSummary; selectedTarget: SubmissionTargetSelection | null; targetPlan: SubmissionTargetPlan | null; requirementSnapshots: JournalRequirementSnapshot[]; selectingTarget: boolean; targetPlanBusyId: string | null; requirementBusyId: string | null; onRecommendationGenerated: () => void; onSelectTarget: (recommendationRunId: string, journalId: string) => void; onAddBackup: (recommendationRunId: string, journalId: string) => void; onPromoteBackup: (selectionId: string, reason: string) => void; onDiscoverRequirements: (selectionId: string) => void; onSaveManualRequirements: (selectionId: string, sourceUrl: string, requirementText: string) => void; onContinue: () => void }) {
  const { locale, text } = useI18n();
