...
interface JournalFitScores { topicScope: number; articleType: number; contentReadiness: number; language: number; targetLevel: number; openAccess: number; }
interface JournalRecommendation { id: string; name: string; nameEn: string; region: "domestic" | "international"; publisher: string; rankSystem: string; rankTier: string; overallFit: number; scores: JournalFitScores; reasons: string[]; rankingSourceUrl: string; homepageUrl: string; openAccessStatus: string; }
interface JournalRecommendationRun { schemaVersion: number; runId: string; workspaceId: string; manuscriptVersion: number; manuscriptHash: string; algorithmVersion: string; catalogVersion: string; catalogVerifiedDate: string; inferredTopic: ResearchTopic; topicBasis: string; maturityScore: number; preferences: JournalMatchPreferences; domestic: JournalRecommendation[]; international: JournalRecommendation[]; schoolRuleStatus: string; limitations: string[]; externalTransmission: "not_performed"; }
...
function JournalMatchStage({ workspace, onContinue }: { workspace: WorkspaceSummary; onContinue: () => void }) {
  ...
  const update = <K extends keyof JournalMatchPreferences>(key: K, value: JournalMatchPreferences[K]) => setPreferences((current) => ({ ...current, [key]: value }));
  async function calculate() {
    setBusy(true); setError(null);
    try { setRun(await invoke<JournalRecommendationRun>("recommend_journals", { workspaceId: workspace.id, preferences })); }
  ...
  return <>
    <PanelHeading kicker={text("步骤 5 / 8 · 期刊匹配", "Step 5 / 8 · Journal match")} title={text("确定投稿目标候选", "Identify submission candidates")} copy={text("使用当前论文版本和公开目录快照，本机计算国内、国际各 3 家。", "Use the current manuscript version and a public directory snapshot to compute three domestic and three international candidates locally.")} />
    <BoundaryNote title={text("计算边界", "Calculation boundary")} copy={text("不联网、不调用模型、不发送论文；学校规则未配置，本次不计入得分。综合适配分不是录用概率。", "No network, model call, or manuscript transfer. School rules are not configured and are excluded. Fit scores are not acceptance odds.")} />
    ...
    </div><button className="primary-button" type="button" disabled={busy} onClick={() => void calculate()}>{busy ? text("正在计算…", "Calculating…") : run ? text("按调整重新计算", "Recalculate with adjustments") : text("计算推荐", "Calculate recommendations")}<Icon name="arrow" /></button></section>
    {error ? <p className="inline-warning"><Icon name="warning" />{error}</p> : null}
    {run ? <><div className="journal-run-meta"><span>{text("本地推荐记录", "Local recommendation run")} {run.runId}</span><span>{text("稿件完备度", "Manuscript readiness")} {run.maturityScore}/100</span><span>{run.catalogVersion} · {run.catalogVerifiedDate}</span></div><div className="journal-columns"><JournalRecommendationList title={text("国内 3 家", "3 domestic journals")} items={run.domestic} locale={locale} /><JournalRecommendationList title={text("国际 3 家", "3 international journals")} items={run.international} locale={locale} /></div><PaneAction label={text("下一步", "Next")} title={text("确认版本与推荐依据", "Confirm version and recommendation basis")} copy={text("推荐记录已绑定当前稿件哈希；接下来可建立本地存证。", "The recommendation run is bound to the current manuscript hash; next you can create a local attestation.")} buttonLabel={text("进入存证", "Continue to attestation")} onClick={onContinue} /></> : null}
  </>;
}

function JournalRecommendationList({ title, items, locale }: { title: string; items: JournalRecommendation[]; locale: Locale }) {
  const { text } = useI18n();
  return <section className="journal-result-group"><h3>{title}</h3><ol>{items.map((item,index)=><li key={item.id}><header><span>{String(index+1).padStart(2,"0")}</span><div><strong>{locale === "en" ? item.nameEn : item.name}</strong><p>{item.publisher}</p></div><em>{item.overallFit}</em></header><div className="journal-tags"><span>{item.rankTier}</span><span>{text("主题", "Topic")} {item.scores.topicScope}</span><span>{text("完备度", "Readiness")} {item.scores.contentReadiness}</span></div><p>{localizeBackendText(locale,item.reasons[0])}</p></li>)}</ol></section>;
}
...
function EvidencePane({ stage, workspace, structureReport, readinessReport, knowledgeBodySnapshot = null, knowledgeBodyRecord = null, attestation = null, submission = null, submissionExport = null, ruleCatalog = [], selectedRulePackIds = [], submissionElementCatalog = null, revisionDraft = null, revisionValues = {}, revisionResult = null, versionHistory = null, selectedVersion = null, versionComparison = null, isComparingVersions = false }: PaneProps) {
  ...
  if (stage === "journals") return <EvidenceFrame kicker={text("目录与算法证据", "Catalog and algorithm evidence")} title={text("公开来源、本地评分", "Public sources, local scoring")}><div className="document-sheet source-sheet"><span className="document-type">LOCAL FIT V1</span><p className="document-title">computer-ai-2025.1</p><dl><div><dt>{text("国内目录", "Domestic directory")}</dt><dd>CCF 2025 · T1/T2/T3</dd></div><div><dt>{text("国际目录", "International directory")}</dt><dd>CCF AI · A/B/C</dd></div><div><dt>{text("评分权重", "Weights")}</dt><dd>{text("主题 35 · 类型 15 · 完备度 20 · 语言 10 · 策略 15 · 开放获取 5", "Topic 35 · Type 15 · Readiness 20 · Language 10 · Strategy 15 · OA 5")}</dd></div><div><dt>{text("学校规则", "School rules")}</dt><dd>{text("未配置；不参与得分", "Not configured; excluded")}</dd></div><div><dt>{text("外部传输", "External transmission")}</dt><dd>{text("未发生", "None")}</dd></div></dl><small>{text("国内与国际目录相互独立；综合分不是录用概率，投稿前须核对期刊官网。", "Domestic and international directories are independent. The fit score is not an acceptance probability; verify the journal website before submission.")}</small></div></EvidenceFrame>;
