import { GuidedButton, GuidanceProvider, highlightGuidanceTarget, journalPrerequisite, useGuidanceDraft, type Prerequisite, type PrerequisiteCheck } from "./ActionGuidance";
import { PackageExportSummary, packageExportLabel, type PackageExportStatus, type TargetSubmissionExport } from "./PackageExportStatus";
import { SourceRecovery } from "./SourceRecovery";
import { StructureReview, type RecognitionObject } from "./StructureReview";
...
import { DeclarationRequirements, DeclarationDetails, type DeclarationRequirement, type DeclarationPlan } from "./DeclarationRequirements";
...
interface StructureReport {
  recognitions?: RecognitionObject[]; reviewId?: string | null;
...
type SubmissionChecklistStatus = "later" | "not_applicable" | "passed" | "missing" | "recommended" | "author_confirmation" | "manual_verification";
export interface SubmissionMaterialChecklistItem { decision?: string | null; decisionReason?: string | null; notApplicableAllowed?: boolean; id: string; label: string; labelEn: string; group: "target" | "manuscript" | "files" | "declarations"; requirement: "required" | "recommended"; status: SubmissionChecklistStatus; detail: string; verification: "automatic" | "file" | "author" | "manual"; materialKind: SubmissionMaterialKind | null; blocking: boolean; confirmable: boolean; sourceUrl: string | null; evidenceExcerpt: string | null; capturedUnixMs: number | null; freshUntilUnixMs: number | null; requiredCount: number; matchedMaterialIds: string[]; declaration?: DeclarationRequirement | null; declarationRequirementId?: string | null; declarationAction?: string | null; }
...
export interface SubmissionMaterialCatalog { exportStatus?: PackageExportStatus; authorInputsReady?: boolean; checkCurrent?: boolean; checkInvalidationReason?: string | null; pendingTasks?: Array<{ id: string; label: string; labelEn: string; state: string; destination: WorkspaceStage; blockingScope: string }>; schemaVersion: number; workspaceId: string; manuscriptVersion: number; materials: SubmissionMaterial[]; checklist: SubmissionMaterialChecklistItem[]; recommendationReady: boolean; targetVerified: boolean; requiredComplete: boolean; targetCheckReady: boolean; workflowStatus: SubmissionWorkflowStatus; requiredTotal: number; requiredCompleted: number; detectedFigureCount?: number; detectedTableCount?: number; declarationPlan?: DeclarationPlan | null; }
...
export interface JournalRequirementItem { id: string; category: string; label: string; labelEn: string; obligation: JournalRequirementObligation; detail: string; sourceUrl: string; evidenceExcerpt: string; declaration?: DeclarationRequirement | null; }
...
interface TargetSubmissionPackageFile { materialId: string | null; displayName: string; relativePath: string; role: string; materialKind: SubmissionMaterialKind | null; checklistItemId: string | null; checklistLabel: string | null; checklistLabelEn?: string | null; inclusionAvailable?: boolean; required: boolean; included: boolean; sizeBytes: number; contentHash: string; validationStatus: string; validationIssues: string[]; }
...
function ManuscriptDockApp() {
  ...
  const [importMatches, setImportMatches] = useState<Array<{ workspace: WorkspaceSummary; exactContent: boolean }>>([]);
    ...
    void invoke<WorkspaceLifecycle>("get_workspace_lifecycle", { workspaceId: workspace.id })
      .then((lifecycle) => {
        ...
        if (lifecycle.readinessReport) {
          setSelectedRulePackIds(restoredRuleSelection(lifecycle.readinessReport.rulePacks));
        }
  ...
  function selectManuscript() {
    ...
    void Promise.resolve()
      ...
      .then(async (result) => {
        if (result.status === "selected") {
          setImportMatches(await invoke<Array<{ workspace: WorkspaceSummary; exactContent: boolean }>>("find_workspace_matches", { selectionId: result.selectionId }));
  ...
  function openStage(stage: WorkspaceStage, itemId?: string) {
    setMaterialFocusId(itemId ?? null);
  ...
  function syncRecommendationAnalysis() {
    ...
    void invoke<WorkspaceLifecycle>("get_workspace_lifecycle", { workspaceId: activeWorkspace.id })
      .then((lifecycle) => {
        ...
        setSubmissionTargetSelection(lifecycle.submissionTarget);
        setSubmissionTargetPlan(lifecycle.submissionTargetPlan);
        setJournalRequirementSnapshots(current => lifecycle.journalRequirements ? [lifecycle.journalRequirements, ...current.filter(item => item.targetSelectionId !== lifecycle.journalRequirements!.targetSelectionId)] : current);
      })
      .catch((error: unknown) => setErrorMessage(normalizeError(error)));
  }
  ...
  async function exportTargetSubmission() {
    ...
    try {
      ...
      if (result) {
        try {
          setSubmissionMaterials(await invoke<SubmissionMaterialCatalog>("get_submission_materials", { workspaceId: activeWorkspace.id }));
        } catch {
          setErrorMessage(text("文件已导出，但状态刷新失败。请重新打开工作区查看已保存的导出记录。", "Files were exported, but the status could not refresh. Reopen the workspace to load the saved export record."));
        }
        refreshTargetSubmissionPackagePlan(activeWorkspace.id);
      }
    } catch (error) {
      ...
      <div className="app-shell landing-shell">
        ...
        <div className="landing-review-layout">
          <nav className="task-rail landing-rail" aria-label={text("工作台导航", "Workspace navigation")}>
            ...
            {WORKSPACE_SECTIONS.filter((section) => section.id !== "overview").map((section) => <GuidedButton key={section.id} className="rail-button" type="button" aria-label={localize(locale, section.zh, section.en)} title={text("点击查看下一步", "Click for the next step")} prerequisite={{ message: text("请先导入稿件并创建本地工作区。", "Import a manuscript and create a local workspace first."), target: ".intake-panel .text-button, .intake-panel .primary-button" }}><Icon name={section.icon} /></GuidedButton>)}
          </nav>
              ...
              <section className="intake-panel" aria-label={text("选择论文稿件", "Select a manuscript")}>
                {!manuscript ? (
                  ...
                  <div className="intake-selected">
                    ...
                    {manuscript.kind === "pdf" ? <p>{text("PDF 可检查，不能直接改写；如需修改，请准备 DOCX/TEX 源稿。", "PDFs can be checked but cannot be rewritten here; prepare a DOCX/TEX source for editing.")}</p> : null}
                    <ExistingWorkspaceMatches key={selectionId} matches={importMatches} onOpen={openRecentWorkspace} />
                    <div className="intake-action-row">
                      <div><span className="field-kicker">{text("创建工作区", "Create workspace")}</span><h3>{text("开始准备这篇论文", "Start preparing this manuscript")}</h3><p>{text("复制并计算内容指纹，源稿件不会被修改。", "Copy and fingerprint the content without modifying the source manuscript.")}</p></div>
                      <GuidedButton className="primary-button" type="button" onClick={createWorkspace} disabled={isCreatingWorkspace} prerequisite={!selectionId && { message: text("请先选择稿件，再创建工作区。", "Choose a manuscript before creating a workspace."), target: ".intake-panel .text-button, .intake-panel .primary-button" }}>{isCreatingWorkspace ? text("正在创建…", "Creating…") : importMatches.length ? text("仍新建独立档案", "Create a separate workspace") : text("保存并开始", "Save and start")}<Icon name="arrow" /></GuidedButton>
                    </div>
                  </div>
                )}
  ...
  return (
    <GuidanceProvider key={activeWorkspace.id} onNavigate={openStage}><div className="app-shell workspace-shell">
      <a className="skip-link" href="#main-content">{text("跳到主要内容", "Skip to main content")}</a>
        ...
        <aside className="workspace-sidebar">
          ...
          <nav className="primary-task-nav" aria-label={text("投稿准备主任务", "Primary submission tasks")}>
            {WORKSPACE_SECTIONS.map((section) => {
              const complete = section.id === "overview" || (section.id === "materials" && authorMaterialInputsReady(submissionMaterials)) || (section.id === "journals" && isSubmissionTargetCurrent(activeWorkspace, submissionTargetSelection) && journalRequirementSnapshotReady(currentJournalRequirementSnapshot(submissionTargetSelection, journalRequirementSnapshots))) || (section.id === "prepare" && Boolean(submissionMaterials?.targetCheckReady)) || (section.id === "submission" && submissionMaterials?.exportStatus?.state === "exported");
              return <button key={section.id} type="button" title={localize(locale, section.zh, section.en)} aria-current={activeSection === section.id ? "page" : undefined} data-complete={complete} onClick={() => openStage(section.stage)}><Icon name={section.icon} /><span>{section.id === "prepare" && locale === "en" ? <>Check<br />revise</> : localize(locale, section.zh, section.en)}</span><small>{section.id === "submission" ? packageExportLabel(submissionMaterials?.exportStatus, submissionMaterials?.targetCheckReady, locale) : complete ? text("已完成", "Done") : text("待处理", "Pending")}</small></button>;
            })}
          </nav>
...
function modelSettingsPrerequisite(slots: ModelSlotDraft[], locale: Locale): Prerequisite | null {
  for (const slot of slots.filter(slot => slot.enabled)) {
    const field = !slot.providerLabel.trim() ? "providerLabel" : !slot.baseUrl.trim() || isDeepSeekDocumentationUrl(slot.baseUrl) ? "baseUrl" : !slot.model.trim() ? "model" : slot.clearApiKey || (!slot.hasApiKey && !slot.apiKey.trim()) ? "apiKey" : null;
    if (field) return {
      message: localize(locale, "请补齐此启用项的服务名称、有效 API 地址、模型名和密钥；也可以停用此项后保存。", "Complete this enabled slot’s provider, valid API URL, model and key, or disable the slot before saving."),
      scope: ".model-settings", target: `[data-model-slot="${slot.role}"] [data-model-field="${field}"]`,
    };
  }
  return null;
}

export function GlobalModelSettingsDialog({ open, onClose, onSaved }: { open: boolean; onClose: () => void; onSaved: (summary: ModelSettingsSummary) => void }) {
  const { locale, text } = useI18n();
    ...
    <section ref={dialogRef} className="global-model-settings-dialog" role="dialog" aria-modal="true" aria-labelledby="global-model-settings-title">
      ...
      {loading ? <p className="model-settings-loading" role="status">{text("正在读取系统凭据状态…", "Reading credential status…")}</p> : <section className="model-settings global-model-settings" aria-label={text("模型槽位设置", "Model slot settings")}>
        ...
        <div className="model-slot-grid">{slotDrafts.map((slot) => {
          ...
          return <fieldset className="model-slot" data-model-slot={slot.role} key={slot.role}>
            ...
            <label>{text("提供方名称", "Provider label")}<input maxLength={100} data-model-field="providerLabel" value={slot.providerLabel} onChange={(event) => updateSlot(slot.role, "providerLabel", event.target.value)} placeholder={text("例如：OpenAI", "e.g. OpenAI")} /></label>
            <label>{text("API 地址", "API base URL")}<input maxLength={500} aria-label={`${modelSlotLabel(slot.role, locale)} ${text("API 地址", "API base URL")}`} data-model-field="baseUrl" value={slot.baseUrl} aria-invalid={isDeepSeekDocumentationUrl(slot.baseUrl)} onChange={(event) => updateSlot(slot.role, "baseUrl", event.target.value)} placeholder="https://api.example.com/v1" inputMode="url" />{isDeepSeekDocumentationUrl(slot.baseUrl) ? <span className="model-field-error" role="alert">{text("这是 DeepSeek 文档页，不是 API。请改用 https://api.deepseek.com", "This is the DeepSeek documentation site, not its API. Use https://api.deepseek.com")}</span> : null}</label>
            <label>{text("模型名称", "Model name")}<input maxLength={200} data-model-field="model" value={slot.model} onChange={(event) => updateSlot(slot.role, "model", event.target.value)} placeholder="model-id" /></label>
            <label>{text("API Key", "API key")}<input maxLength={2000} aria-label={`${modelSlotLabel(slot.role, locale)} API Key`} type="password" autoComplete="new-password" data-model-field="apiKey" value={slot.apiKey} aria-invalid={slot.enabled && !slot.hasApiKey && !slot.apiKey.trim()} onChange={(event) => { updateSlot(slot.role, "apiKey", event.target.value); updateSlot(slot.role, "clearApiKey", false); }} placeholder={slot.hasApiKey ? text("已安全保存；留空表示保留", "Stored securely; leave blank to retain") : text("输入后保存到系统凭据库", "Saved to the system credential store")} />{slot.enabled && !slot.hasApiKey && !slot.apiKey.trim() ? <span className="model-field-error" role="alert">{text("启用模型需要 API Key；输入后点击“保存模型设置”。", "An API key is required. Enter it, then save the model settings.")}</span> : null}</label>
            <label className="model-clear-key"><input type="checkbox" checked={slot.clearApiKey} onChange={(event) => updateSlot(slot.role, "clearApiKey", event.target.checked)} disabled={!slot.hasApiKey} />{text("删除已保存的 Key", "Delete saved key")}{!slot.hasApiKey && <small>{text("尚无已保存密钥，无需清除", "No saved key to delete")}</small>}</label>
          </fieldset>;
        })}</div>
        <div className="model-settings-actions"><p>{text("Key 由 Rust 写入操作系统凭据库，界面不会读取或回显明文。同一次应用运行中，每个已保存 Key 只向系统凭据库读取一次；点击具体模型任务即完成该次授权，不再重复勾选。", "Rust stores keys in the operating system credential store, and the interface never reads or reveals plaintext. During one app session, each stored key is read from the credential store only once; clicking a specific model task authorizes that call without another checkbox.")}</p><GuidedButton className="primary-button" type="button" disabled={saving} prerequisite={invalidEnabledDraft && modelSettingsPrerequisite(slotDrafts, locale)} onClick={() => void saveSettings()}>{saving ? text("保存中…", "Saving…") : text("保存模型设置", "Save model settings")}</GuidedButton></div>
      </section>}
    ...
    {visibleWorkspaces.length > 0 ? <ul className="recent-list">{visibleWorkspaces.slice(0, 20).map((workspace) => {
      ...
      return <li key={workspace.id}>
        <div className="workspace-row">
          <GuidedButton className="workspace-open-button" type="button" onClick={() => onOpen(workspace)} disabled={isBusy} prerequisite={archived && { message: text("该工作区已归档。请先在管理菜单中恢复到最近工作区。", "This workspace is archived. Restore it to Recent workspaces from its management menu first."), prepare: () => setMenuWorkspaceId(workspace.id), scope: ".recent-list > li", target: ".workspace-management-menu button:nth-child(2)" }} aria-label={archived ? text(`${workspace.manuscript.name} 已归档`, `${workspace.manuscript.name} is archived`) : text(`打开 ${workspace.manuscript.name}`, `Open ${workspace.manuscript.name}`)}><span className="recent-file-icon" aria-hidden="true"><Icon name="file" /></span><span className="recent-file-copy"><strong>{workspace.manuscript.name}</strong><span>{archived ? text("已归档 · 恢复后可继续工作", "Archived · Restore to continue") : text("源快照", "Source snapshot")} {archived ? "" : `v${workspace.snapshotVersion} · ${formatModifiedDate(workspace.importedUnixMs, locale)}`}</span></span><code>{workspace.contentHash.slice(0, 8)}</code>{!archived ? <Icon name="arrow" /> : null}</GuidedButton>
...
function authorMaterialInputsReady(catalog: SubmissionMaterialCatalog | null) {
  if (!catalog?.targetVerified) return false;
  if (catalog.authorInputsReady !== undefined) return catalog.authorInputsReady;
...
function SubmissionFlowRail({ catalog }: { catalog: SubmissionMaterialCatalog | null }) {
  ...
  const steps = [
    ...
    { id: "verified", label: text("目标核验", "Target verification"), copy: text("期刊、文章类型与官方要求", "Journal, article type, and official rules"), complete: Boolean(catalog?.targetVerified), active: catalog?.workflowStatus === "preliminary_recommendation" },
    { id: "materials", label: text("材料准备", "Materials preparation"), copy: text("必需文件与作者确认", "Required files and author confirmations"), complete: Boolean(catalog?.requiredComplete), active: catalog?.workflowStatus === "target_verified" || catalog?.workflowStatus === "materials_required" },
    { id: "ready", label: text("投稿包预检", "Package preflight"), copy: text("完成当前目标检查", "Current target check complete"), complete: Boolean(catalog?.targetCheckReady), active: catalog?.workflowStatus === "materials_complete_check_required" },
  ];
...
export function restoredRuleSelection(packs: Array<{ id: string }>): string[] {
  const internal = new Set(["md.core.structure", "md.stage.initial-submission", "core-structure-v1", "initial-submission-v1"]);
  return [...new Set(packs.map(pack => pack.id).filter(id => !internal.has(id)))];
}

export function ExistingWorkspaceMatches({ matches, onOpen }: { matches: Array<{ workspace: WorkspaceSummary; exactContent: boolean }>; onOpen: (workspace: WorkspaceSummary) => void }) {
  const { locale, text } = useI18n();
  const [expanded, setExpanded] = useState(false);
  if (matches.length === 0) return null;
  const visibleMatches = expanded ? matches : matches.slice(0, 3);
  const remaining = new Intl.NumberFormat(locale).format(matches.length - 3);
  return <section className="pending-tasks" aria-label={text("发现已有本地档案", "Existing local workspaces found")}>
    <h3>{text("发现已有本地档案", "Existing local workspaces found")}</h3>
    <div id="existing-workspace-match-list">{visibleMatches.map(match => <div key={match.workspace.id}>
      <strong>{match.workspace.manuscript.name} · v{match.workspace.snapshotVersion}</strong>
      <p>{match.exactContent ? text("内容相同，无需重复新建。", "Identical content; no new workspace is needed.") : text("名称相同、内容不同。可进入已有档案，在版本管理中添加修改稿。", "Same name, different content. Open the existing workspace and add a revision in version management.")} · {formatModifiedDate(match.workspace.importedUnixMs, locale)}</p>
      <button className="secondary-button" type="button" onClick={() => onOpen(match.workspace)}>{text("继续已有档案", "Continue existing workspace")}</button>
    </div>)}</div>
    {matches.length > 3 && <button className="text-button" type="button" aria-expanded={expanded} aria-controls="existing-workspace-match-list" onClick={() => setExpanded(current => !current)}>
      {expanded ? text("收起", "Show fewer") : text(`展开更多（另 ${remaining} 个）`, `Show more (${remaining} remaining)`)}
    </button>}
  </section>;
}

export function PendingTasks({ catalog, onOpenStage }: { catalog: SubmissionMaterialCatalog | null; onOpenStage: (stage: WorkspaceStage, itemId?: string) => void }) {
  const { locale, text } = useI18n();
  const tasks = catalog?.pendingTasks ?? [];
  if (tasks.length === 0) return null;
  return <section className="pending-tasks" aria-label={text("当前待办", "Current tasks")}><h3>{text(`投稿前还有 ${tasks.length} 项待办`, `${tasks.length} tasks before submission`)}</h3><p>{text("这些事项影响最终导出；本地基础检查随时可运行。", "These tasks affect final export. Local basic checks remain available.")}</p><ul>{tasks.map(task => <li key={task.id}><button type="button" onClick={() => onOpenStage(task.destination, task.id)}>{locale === "en" && task.labelEn ? task.labelEn : localizeBackendText(locale, task.label)}<span aria-hidden="true"> →</span></button></li>)}</ul></section>;
}
...
type OperationPaneProps = PaneProps & {
  ...
  onAnalyze: () => void; onEvaluate: () => void; onToggleRulePack: (rulePackId: string) => void; onOpenStage: (stage: WorkspaceStage, itemId?: string) => void;
  ...
  onMaterialCatalogUpdated: (catalog: SubmissionMaterialCatalog) => void;
...
function OperationPane(props: OperationPaneProps) {
  ...
  const exportTask = submissionMaterials?.pendingTasks?.[0];
  const exportPrerequisite = (): Prerequisite | null => targetSubmissionPackagePlan?.ready && journalRequirementSnapshotReady(targetRequirements) ? null : !(targetCurrent && journalRequirementSnapshotReady(targetRequirements)) ? journalPrerequisite(text("需要先到“目标期刊”确认当前目标及有效官方要求。", "Confirm the current target and official requirements in Target Journal first."), text("前往目标期刊", "Go to Target Journal")) : {
    message: text("投稿包尚有待办：", "The package still has unresolved tasks: ") + (targetSubmissionPackagePlan?.blockers.map(message => localizeBackendText(locale, message)).join("；") || (exportTask ? localize(locale, exportTask.label, exportTask.labelEn) : text("请重新检查当前稿件与材料。", "Recheck the current manuscript and materials."))),
    destination: exportTask?.destination ?? "check", itemId: exportTask?.id,
    target: exportTask?.destination === "materials" ? (exportTask.id ? `[id=${JSON.stringify(`requirement-${exportTask.id}`)}]` : "#material-panel-requirements") : "#operation-pane",
    actionLabel: exportTask ? text(`前往处理：${exportTask.label}`, `Resolve: ${exportTask.labelEn}`) : text("前往检查与修订", "Go to Check and revise"),
  };
  ...
  if (stage === "source") return <><WorkspaceOverview workspace={workspace} readinessReport={readinessReport} materials={submissionMaterials} target={submissionTargetSelection} requirementSnapshots={journalRequirementSnapshots} submission={submission} onOpenStage={onOpenStage} /><SourceRecovery workspaceId={workspace.id} isPdf={workspace.manuscript.kind === "pdf"} onOpenWorkspace={props.onOpenLinkedWorkspace} /></>;

  if (stage === "materials") return <SubmissionMaterialsCenter focusItemId={props.materialFocusId} workspace={workspace} catalog={submissionMaterials} busy={isAddingMaterials} confirmingRequirementId={confirmingRequirementId} targetReady={targetRequirementsReady} checkTargetReady={() => targetCurrent && journalRequirementSnapshotReady(targetRequirements)} onAdd={onAddMaterial} onSetIncluded={onSetMaterialIncluded} onDelete={onDeleteMaterial} onConfirm={onConfirmSubmissionRequirement} onCatalogUpdated={props.onMaterialCatalogUpdated} onContinue={() => onOpenStage(targetRequirementsReady ? "check" : "journals")} />;
  ...
  if (stage === "check") {
    ...
    if (!readinessReport) return <><StructureCheckSummary report={structureReport} /><StructureReview key={`${workspace.id}-${structureReport.reviewId}`} report={structureReport} onSaved={onAnalyze} /><TargetRuleSelector ruleCatalog={ruleCatalog} selectedRulePackIds={selectedRulePackIds} loading={isLoadingRuleCatalog} structureReady onToggle={onToggleRulePack} onContinue={onEvaluate} actionLabel={isEvaluating ? text("正在检查…", "Checking…") : text("运行本地基础检查", "Run local basic checks")} disabled={isEvaluating} /></>;
    return <><StructureReview key={`${workspace.id}-${structureReport.reviewId}`} report={structureReport} onSaved={onAnalyze} /><CheckRequirementReviews focusItemId={props.materialFocusId} workspaceId={workspace.id} catalog={submissionMaterials} onUpdated={props.onMaterialCatalogUpdated} disabled={isEvaluating} /><PendingTasks catalog={submissionMaterials} onOpenStage={onOpenStage} /><PanelHeading kicker={`${text("主任务 4 · 投稿检查", "Primary task 4 · Submission check")} · v${readinessReport.reportVersion}`} title={submissionMaterials?.targetCheckReady ? text("检查与核验已完成", "Checks and reviews complete") : outcomeLabel(readinessReport.outcome, locale)} copy={text("这是本地通用规则检查。期刊专属要求与作者核验在待办中单独列出；运行完成不代表可投稿。", "These are local general-rule checks. Journal requirements and author reviews appear separately in the task list; completing a run does not mean ready to submit.")} /><div className="metric-row" aria-label={text("投稿检查统计", "Submission-check metrics")}><Metric label={text("通过", "Passed")} value={readinessReport.passedCount} /><Metric label={text("建议", "Suggestions")} value={readinessReport.warningCount} /><Metric label={text("阻断", "Blocked")} value={readinessReport.blockedCount} /><Metric label={text("待确认", "Confirmations")} value={readinessReport.findings.filter(f => f.status === "confirmation" && !confirmationReviewed(submissionMaterials, f.ruleId)).length} /></div><ol className="finding-list" aria-label={text("投稿检查明细", "Submission-check details")}>{readinessReport.findings.map((finding) => <li key={finding.ruleId} data-status={finding.status}><span className="finding-status">{finding.status === "confirmation" && confirmationReviewed(submissionMaterials, finding.ruleId) ? text("已核验", "Reviewed") : findingLabel(finding.status, locale)}</span><div><strong>{locale === "en" && finding.messageEn ? finding.messageEn : localizeBackendText(locale, finding.message)}</strong><details><summary>{text("查看检查位置", "View check location")}</summary><code>{finding.sourceLocation}</code></details></div></li>)}</ol><div className="secondary-action-row"><button className="text-button" type="button" onClick={onEvaluate} disabled={isEvaluating}>{text("重新检查", "Run again")}</button></div><PaneAction label={text("下一步", "Next")} title={submissionMaterials?.targetCheckReady ? text("查看投稿包预检", "Review package preflight") : !authorMaterialInputsReady(submissionMaterials) ? text("处理当前待办", "Resolve current tasks") : text("根据结论修订", "Revise from findings")} copy={submissionMaterials?.targetCheckReady ? text("当前检查与材料一致，可以进入投稿包核对并导出。", "Checks match the current materials. Review and export the package next.") : !authorMaterialInputsReady(submissionMaterials) ? text("按待办核验目标、补充文件或核对要求，再重新检查当前材料。", "Verify the target, add files, or review requirements listed in the tasks, then check the current materials again.") : text("进入安全字段修订台；每次保存都会生成新版本并自动复查。", "Open safe-field revision; every save creates a new version and automatically rechecks it.")} buttonLabel={submissionMaterials?.targetCheckReady ? text("查看投稿包", "Review package") : !authorMaterialInputsReady(submissionMaterials) ? text("处理当前待办", "Resolve current tasks") : text("进入修订", "Continue to revision")} onClick={() => onOpenStage(submissionMaterials?.targetCheckReady ? "submission" : authorMaterialInputsReady(submissionMaterials) ? "revision" : submissionMaterials?.targetVerified ? "materials" : "journals")} /></>;
  }
  ...
  if (stage === "revision") {
    ...
    return <><SourceRecovery workspaceId={workspace.id} isPdf={workspace.manuscript.kind === "pdf"} onOpenWorkspace={props.onOpenLinkedWorkspace} /><SubmissionElementsDesk catalog={submissionElementCatalog} draft={revisionDraft} values={revisionValues} result={revisionResult} loading={isLoadingSubmissionElements} saving={isApplyingRevision} selectedPublisherCount={ruleCatalog.filter((item) => item.category === "publisher" && selectedRulePackIds.includes(item.id)).length} onValueChange={onRevisionValueChange} onSave={onApplyRevision} onContinue={() => onOpenStage("submission")} /></>;
  }
  ...
  if (!knowledgeBodyRecord?.disciplineClassification || requiresUpdatedSnapshot) {
    ...
    return <>
      ...
      <KnowledgeDialoguePanel workspace={workspace} knowledgeBodyRecord={null} completionPrerequisite={{ message: text("请先审核候选内容、选择学科并完成知识体确认。", "Review candidates, choose a discipline and finalize the knowledge body first."), target: "#knowledge-finalize" }} />
      ...
      <label className="confirmation-control knowledge-review-attestation">
        <input type="checkbox" id="knowledge-final-confirm" checked={knowledgeReviewConfirmed} disabled={!reviewComplete} onChange={(event) => onKnowledgeReviewConfirmed(event.target.checked)} />
      ...
      {!reviewComplete ? <GuidedButton className="text-button" prerequisite={candidates.length ? { message: text("请先核对每条候选内容，再确认最终知识体。", "Review each candidate before confirming the final knowledge body."), target: '[data-decision="pending"] button' } : { message: text("没有可核验候选，请重新提取结构。", "No candidates are available. Extract the structure again."), destination: "check", actionLabel: text("前往检查与修订", "Go to Check and revise"), target: "#operation-pane" }}>{text("查看尚未完成的核验", "Review unfinished candidates")}</GuidedButton> : null}
      ...
      <GuidedButton className="primary-button" type="button" id="knowledge-finalize" disabled={isLoadingDisciplineCatalog || isFinalizingKnowledge} prerequisite={candidates.length === 0 ? { message: text("尚无可审核候选，请到检查与修订重新提取结构。", "No reviewable candidates are available. Extract the structure again in Check and revise."), destination: "check", actionLabel: text("前往检查与修订", "Go to Check and revise"), target: "#operation-pane" } : !reviewComplete ? { message: text("请先逐条选择纳入或排除候选内容。", "Choose Include or Exclude for every candidate first."), target: '[data-decision="pending"] button' } : !selectedDisciplineCode ? { message: text("请选择主要学科分类。", "Choose the primary discipline."), target: "#discipline-code" } : !knowledgeReviewConfirmed ? { message: text("请核对并确认最终纳入的研究知识。", "Review and confirm the research knowledge to include."), target: "#knowledge-final-confirm" } : null} onClick={onFinalizeKnowledge}>{isFinalizingKnowledge ? text("正在固化…", "Finalizing…") : requiresUpdatedSnapshot ? text("确认审核并更新快照", "Confirm review and update snapshot") : text("确认审核并固化知识体", "Confirm review and finalize")}<Icon name="arrow" /></GuidedButton>
    </>;
  }
...
function WorkspaceOverview({ workspace, readinessReport, materials, target, requirementSnapshots, submission, onOpenStage }: { workspace: WorkspaceSummary; readinessReport: ReadinessReport | null; materials: SubmissionMaterialCatalog | null; target: SubmissionTargetSelection | null; requirementSnapshots: JournalRequirementSnapshot[]; submission: SubmissionRecord | null; onOpenStage: (stage: WorkspaceStage, itemId?: string) => void }) {
  ...
  const nextStage: WorkspaceStage = !targetReady ? "journals" : !authorMaterialInputsReady(materials) ? "materials" : !materials?.targetCheckReady ? "check" : "submission";
  ...
  return <>
    ...
    <button className="secondary-button" type="button" onClick={() => onOpenStage("check")}>{text("直接检查主稿（无需官网或模型）", "Check manuscript now (no website or model needed)")}</button>
    ...
    <PaneAction label={text("建议下一步", "Recommended next step")} title={nextStage === "submission" && materials?.exportStatus?.state === "exported" ? text("查看已导出的投稿包", "View exported package") : localize(locale, nextLabels[nextStage][0], nextLabels[nextStage][1])} copy={text("系统根据当前论文记录引导下一项主任务，你也可以从左侧直接切换。", "The app suggests the next primary task from the current manuscript record; you can also switch from the sidebar.")} buttonLabel={nextStage === "submission" && materials?.exportStatus?.state === "exported" ? text("查看投稿包", "View package") : localize(locale, nextLabels[nextStage][0], nextLabels[nextStage][1])} onClick={() => onOpenStage(nextStage)} />
    <PackageExportSummary status={materials?.exportStatus} ready={materials?.targetCheckReady} />
    <PendingTasks catalog={materials} onOpenStage={onOpenStage} />
  </>;
}

function submissionChecklistCopy(item: SubmissionMaterialChecklistItem, locale: Locale) {
  if (item.declaration) return { label: locale === "en" ? item.labelEn : item.label, detail: localizeBackendText(locale, item.detail) };
  if (locale === "zh-CN" || item.detail === "REQUIREMENT_COMPARE_SOURCE" || item.detail === "CHECK_AUTHOR_REVIEW" || item.detail === "REQUIREMENT_PRESENCE_CHECK") return { label: locale === "en" ? item.labelEn : item.label, detail: localizeBackendText(locale, item.detail) };
...
function submissionChecklistStatusLabel(status: SubmissionChecklistStatus, locale: Locale) {
  if (status === "later") return localize(locale, "后续阶段", "Later stage");
  if (status === "not_applicable") return localize(locale, "不适用", "Not applicable");
...
function materialAcceptedFormats(kind: SubmissionMaterialKind, locale: Locale, officialFormats?: string[]) {
  if (officialFormats?.length) return officialFormats.join(locale === "zh-CN" ? "、" : ", ").toUpperCase();
  const formats: Record<SubmissionMaterialKind, [string, string]> = {
    ...
    declaration: ["DOC、DOCX、ODT、RTF、TEX、PDF、TXT、PNG、JPG、TIFF", "DOC, DOCX, ODT, RTF, TEX, PDF, TXT, PNG, JPG, TIFF"],
...
function SubmissionPreparationTree({ workspace, catalog }: { workspace: WorkspaceSummary; catalog: SubmissionMaterialCatalog | null }) {
  ...
  const includedMaterials = currentMaterials.filter((material) => material.included && checklist.some((item) => item.id === material.checklistItemId && item.status !== "later" && item.status !== "not_applicable") && (!catalog?.declarationPlan || (material.targetSelectionId === catalog.declarationPlan.targetSelectionId && material.requirementSnapshotId === catalog.declarationPlan.requirementSnapshotId)));
  const includedFileCount = new Set(includedMaterials.map((material) => checklist.find((item) => item.id === material.checklistItemId)?.declaration?.sharedFileAllowed ? `${material.contentHash}.${material.extension}` : material.materialId)).size;
...
export function HistoricalMaterialReuse({ workspaceId, material, items, onUpdated }: { workspaceId: string; material: SubmissionMaterial; items: SubmissionMaterialChecklistItem[]; onUpdated: (catalog: SubmissionMaterialCatalog) => void }) {
  const { locale, text } = useI18n();
  ...
  async function reuse() {
    ...
    try { onUpdated(await invoke<SubmissionMaterialCatalog>("reuse_previous_material", { workspaceId, materialId: material.materialId, checklistItemId: itemId })); }
    catch (reason) { setError(localizeBackendText(locale, String(reason))); }
    finally { setBusy(false); }
  }
  return <details className="historical-material-reuse"><summary>{text("核对后沿用此文件", "Review and reuse this file")}</summary><p>{text("选择当前要求后建立新绑定；请先确认文件适用于当前稿件，原记录保留。", "Choose the current requirement to create a new binding. Check that the file applies to this manuscript; the original record remains.")}</p><label>{text("绑定到当前要求", "Bind to current requirement")}<select value={itemId} onChange={event => setItemId(event.target.value)}><option value="">{text("请选择", "Select a requirement")}</option>{items.filter(item => item.verification === "file" && item.materialKind === material.kind && item.status !== "not_applicable" && item.status !== "later").map(item => <option key={item.id} value={item.id}>{locale === "en" ? item.labelEn : item.label}</option>)}</select></label><GuidedButton type="button" disabled={busy} prerequisite={!itemId && { message: items.some(item => item.verification === "file" && item.materialKind === material.kind && item.status !== "not_applicable" && item.status !== "later") ? text("请选择此文件对应的当前上传项，并核对适用性。", "Select the current upload requirement and verify this file applies.") : text("当前没有同类型的适用上传项，请先核对当前要求，或为相应要求重新上传文件。", "No applicable upload requirement matches this file type. Review current requirements or upload a file for the relevant requirement."), target: items.some(item => item.verification === "file" && item.materialKind === material.kind && item.status !== "not_applicable" && item.status !== "later") ? "select" : "#material-panel-requirements", scope: items.some(item => item.verification === "file" && item.materialKind === material.kind && item.status !== "not_applicable" && item.status !== "later") ? ".historical-material-reuse" : undefined, prepare: items.some(item => item.verification === "file" && item.materialKind === material.kind && item.status !== "not_applicable" && item.status !== "later") ? undefined : () => document.getElementById("material-view-requirements")?.click(), actionLabel: text("前往要求清单", "Go to Requirements") }} onClick={() => void reuse()}>{text("确认适用并沿用", "Confirm applicability and reuse")}</GuidedButton>{error ? <p role="alert">{error}</p> : null}</details>;
}

export function SubmissionMaterialsCenter({ focusItemId, workspace, catalog, busy, confirmingRequirementId, targetReady, checkTargetReady, onAdd, onSetIncluded, onDelete, onConfirm, onCatalogUpdated, onContinue }: { focusItemId?: string | null; onCatalogUpdated: (catalog: SubmissionMaterialCatalog) => void; workspace: WorkspaceSummary; catalog: SubmissionMaterialCatalog | null; busy: boolean; confirmingRequirementId: string | null; targetReady: boolean; checkTargetReady?: () => boolean; onAdd: (kind: SubmissionMaterialKind, checklistItemId?: string) => void; onSetIncluded: (materialId: string, included: boolean) => void; onDelete: (materialId: string) => void; onConfirm: (itemId: string, confirmed: boolean) => void; onContinue: () => void }) {
  const { locale, text } = useI18n();
  const [deleteMaterialId, setDeleteMaterialId] = useState<string | null>(null);
  const [activeView, setActiveView] = useState<SubmissionMaterialView>("overview");
  useEffect(() => {
    if (focusItemId) setActiveView("requirements");
  }, [focusItemId]);
  useEffect(() => {
    if (activeView !== "requirements" || !focusItemId) return;
    const element = document.getElementById(`requirement-${focusItemId}`);
    element?.scrollIntoView?.({ block: "center" });
    element?.focus();
    if (element) return highlightGuidanceTarget(element);
  }, [activeView, focusItemId]);
  ...
  const fileUploadItems = (catalog?.checklist ?? []).filter((item) => item.verification === "file" && item.materialKind && item.status !== "not_applicable");
  const figureAndTableItems = fileUploadItems.filter((item) => item.materialKind === "figure" || item.materialKind === "table");
  const targetMaterialItems = fileUploadItems.filter((item) => !item.id.startsWith("common-") && item.materialKind !== "figure" && item.materialKind !== "table");
  const commonMaterialItems = fileUploadItems.filter((item) => item.id.startsWith("common-"));
  ...
  const materialViews: Array<{ id: SubmissionMaterialView; label: string; hint: string; count: string }> = [
    ...
    { id: "upload", label: text("上传资料", "Upload"), hint: text("全部文件入口", "All file entries"), count: String(fileUploadItems.length) },
  ...
  return <>
    ...
    {activeView === "upload" ? <section id="material-panel-upload" role="tabpanel" aria-labelledby="material-view-upload" className="material-view-panel">
      <button className="text-button" type="button" onClick={() => setActiveView("requirements")}>{text("核验或补充声明要求", "Verify or add declaration requirements")}</button><p className="material-replacement-policy"><Icon name="review" /><span>{text("上传时会检查当前稿件、当前期刊和同一上传项中的同名文件；发现同名文件后，必须确认才会替换工作区副本。", "Uploads are checked for duplicate names within the current manuscript, journal, and upload slot. Replacing an existing workspace copy always requires confirmation.")}</span></p>
      {fileUploadItems.length === 0 ? <div className="upload-empty-state"><h3>{text("上传资料", "Upload materials")}</h3><p>{targetReady ? text("当前有效要求未生成上传项。请先核对要求清单；无需为不存在的要求补文件。", "The current valid requirements contain no upload entries. Review the checklist; files are not needed for absent requirements.") : text("上传入口会按当前期刊要求生成。请先确认投稿目标和官方要求。", "Upload entries are generated from the current journal’s requirements. Confirm the target and official requirements first.")}</p><GuidedButton className="secondary-button" disabled={busy} prerequisite={() => !(checkTargetReady?.() ?? targetReady) ? journalPrerequisite(text("需要先到“目标期刊”选择当前版本主投期刊并取得有效官方要求。", "Select a primary journal for this version and capture current official requirements in Target Journal first."), text("前往目标期刊", "Go to Target Journal")) : { message: text("请核对官方要求，确认需要哪些文件。", "Review the official requirements to confirm which files are needed."), prepare: () => setActiveView("requirements"), target: "#material-panel-requirements" }}>{targetReady ? text("核对上传要求", "Review upload requirements") : text("选择文件", "Choose files")}</GuidedButton></div> : null}
      {figureAndTableItems.length > 0 ? <section className="material-upload-hub" aria-labelledby="material-upload-hub-heading"><header><div><span>{text("独立上传入口", "Dedicated upload entry")}</span><h3 id="material-upload-hub-heading">{text("图表文件", "Figures and tables")}</h3></div><strong>{text("按类型分开", "Separated by type")}</strong></header><div className="material-type-grid">{figureAndTableItems.map((item) => { const copy = submissionChecklistCopy(item, locale); const isFigure = item.materialKind === "figure"; return <article key={`upload-${item.id}`}><div><Icon name="file" /><span>{item.matchedMaterialIds.length}/{item.requiredCount}</span></div><h3>{isFigure ? text("上传原始图件", "Upload original figures") : text("上传可编辑表格", "Upload editable tables")}</h3><p>{materialAcceptedFormats(item.materialKind!, locale, item.declaration?.allowedExtensions)}</p><GuidedButton className="secondary-button" type="button" disabled={busy} prerequisite={() => !(checkTargetReady?.() ?? targetReady) && journalPrerequisite(text("需要先到“目标期刊”确认当前版本的主投期刊，并获取有效官方要求。", "Confirm a primary journal for this version and capture current official requirements in Target Journal first."), text("前往目标期刊", "Go to Target Journal"))} aria-label={text(`为${copy.label}上传${isFigure ? "原始图件" : "可编辑表格"}`, `Upload ${isFigure ? "original figures" : "editable tables"} for ${copy.label}`)} onClick={() => onAdd(item.materialKind!, item.id)}>{busy ? text("正在添加…", "Adding…") : isFigure ? text("选择图片文件", "Choose figure files") : text("选择表格文件", "Choose table files")}</GuidedButton></article>; })}</div></section> : null}
      {targetMaterialItems.length > 0 ? <section className="material-upload-hub target-material-upload-hub" aria-labelledby="target-material-upload-heading"><header><div><span>{text("从当前清单同步 · 必需项优先", "Synced from the current checklist · Required first")}</span><h3 id="target-material-upload-heading">{text("当前期刊与主稿附件", "Current journal and manuscript files")}</h3></div><strong>{text(`${targetMaterialItems.length} 个入口`, `${targetMaterialItems.length} entries`)}</strong></header><p className="material-upload-hub-copy">{text("完整工程、匿名主稿以及期刊专属标题页、投稿信、声明和补充文件都会在这里保留上传入口，不必返回要求清单寻找。", "Complete projects, blinded manuscripts, and journal-specific title pages, cover letters, declarations, and supplementary files remain available here without returning to the requirements list.")}</p><div className="material-type-grid common-material-type-grid">{targetMaterialItems.map((item) => { const copy = submissionChecklistCopy(item, locale); return <article key={`target-upload-${item.id}`}><div><Icon name="file" /><span>{item.matchedMaterialIds.length}/{item.requiredCount} · {item.blocking ? text("必需", "Required") : text("按需", "As needed")}</span></div><h3>{copy.label}</h3><p>{copy.detail}</p><DeclarationDetails item={item} /><small>{text("支持：", "Accepted: ")}{materialAcceptedFormats(item.materialKind!, locale, item.declaration?.allowedExtensions)}</small><GuidedButton className="secondary-button" type="button" disabled={busy} prerequisite={() => !(checkTargetReady?.() ?? targetReady) && journalPrerequisite(text("需要先到“目标期刊”确认当前版本的主投期刊，并获取有效官方要求。", "Confirm a primary journal for this version and capture current official requirements in Target Journal first."), text("前往目标期刊", "Go to Target Journal"))} aria-label={text(`为${copy.label}选择文件`, `Choose files for ${copy.label}`)} onClick={() => onAdd(item.materialKind!, item.id)}>{busy ? text("正在添加…", "Adding…") : item.matchedMaterialIds.length > 0 ? text("添加或替换文件", "Add or replace files") : text("选择文件", "Choose files")}</GuidedButton></article>; })}</div></section> : null}
      {commonMaterialItems.length > 0 ? <section className="material-upload-hub common-material-upload-hub" aria-labelledby="common-material-upload-heading"><header><div><span>{text("按需补充 · 不默认设为必需", "Add as needed · Not required by default")}</span><h3 id="common-material-upload-heading">{text("常见投稿附件", "Common submission attachments")}</h3></div><strong>{text(`${commonMaterialItems.length} 类`, `${commonMaterialItems.length} types`)}</strong></header><p className="material-upload-hub-copy">{text("声明、说明及其他附件在不同期刊中的要求差异很大。这里始终提供分类入口；若官方作者指南明确要求，以带官方证据的动态清单为准。", "Journals vary widely in their declaration, explanation, and supporting-file requirements. These categorized upload entries remain available; when the official author guide is explicit, follow the evidence-backed dynamic checklist.")}</p><div className="material-type-grid common-material-type-grid">{commonMaterialItems.map((item) => { const copy = submissionChecklistCopy(item, locale); return <article key={`common-upload-${item.id}`}><div><Icon name="file" /><span>{text(`${item.matchedMaterialIds.length} · 可选`, `${item.matchedMaterialIds.length} · Optional`)}</span></div><h3>{copy.label}</h3><p>{copy.detail}</p><DeclarationDetails item={item} /><small>{text("支持：", "Accepted: ")}{materialAcceptedFormats(item.materialKind!, locale, item.declaration?.allowedExtensions)}</small><GuidedButton className="secondary-button" type="button" disabled={busy} prerequisite={() => !(checkTargetReady?.() ?? targetReady) && journalPrerequisite(text("需要先到“目标期刊”确认当前版本的主投期刊，并获取有效官方要求。", "Confirm a primary journal for this version and capture current official requirements in Target Journal first."), text("前往目标期刊", "Go to Target Journal"))} aria-label={text(`上传${copy.label}`, `Upload ${copy.label}`)} onClick={() => onAdd(item.materialKind!, item.id)}>{busy ? text("正在添加…", "Adding…") : text("选择文件", "Choose files")}</GuidedButton></article>; })}</div></section> : null}
    </section> : null}
    {activeView === "requirements" ? <section id="material-panel-requirements" role="tabpanel" aria-labelledby="material-view-requirements" className="material-view-panel materials-checklist"><header><div><span>{text("动态投稿清单", "Dynamic submission checklist")}</span><h3 id="materials-checklist-heading">{authorInputsReady ? text("作者侧必需资料与确认已完成", "Author-supplied files and confirmations are complete") : text("仍有必需项待处理", "Required items still need attention")}</h3></div><strong>{catalog ? `${catalog.requiredCompleted ?? 0}/${catalog.requiredTotal ?? 0}` : `v${workspace.snapshotVersion}`}</strong></header>{catalog ? <DeclarationRequirements key={`${catalog.workspaceId}-${catalog.manuscriptVersion}-${catalog.declarationPlan?.targetSelectionId}-${catalog.declarationPlan?.requirementSnapshotId}`} catalog={catalog} disabled={busy} prerequisite={() => !(checkTargetReady?.() ?? targetReady) ? journalPrerequisite(text("需要先到“目标期刊”确认主投期刊及有效官方要求。", "Confirm the primary journal and current official requirements in Target Journal first."), text("前往目标期刊", "Go to Target Journal")) : null} onUpdated={onCatalogUpdated} /> : null}<div className="checklist-groups">{groups.map((group) => { const items = (catalog?.checklist ?? []).filter((item) => item.group === group && !item.id.startsWith("common-")); if (items.length === 0) return null; return <section className="checklist-group" key={group} aria-labelledby={`checklist-group-${group}`}><header><h4 id={`checklist-group-${group}`}>{localize(locale, ...groupLabels[group])}</h4><span>{items.length}</span></header><ul>{items.map((item) => { const copy = submissionChecklistCopy(item, locale); return <li id={`requirement-${item.id}`} tabIndex={-1} key={item.id} data-status={item.status}><Icon name={item.status === "passed" ? "check" : "warning"} /><div><div className="checklist-item-heading"><strong>{copy.label}</strong><span>{submissionChecklistStatusLabel(item.status, locale)}</span></div><p>{copy.detail}</p><DeclarationDetails item={item} />{item.requiredCount > 0 ? <small className="material-slot-count">{text(`已绑定 ${item.matchedMaterialIds.length}/${item.requiredCount} 个文件`, `${item.matchedMaterialIds.length}/${item.requiredCount} file(s) bound`)}</small> : null}{item.evidenceExcerpt ? <details className="checklist-evidence"><summary>{text("查看官方依据", "View official evidence")}</summary><blockquote>{item.evidenceExcerpt}</blockquote><a href={item.sourceUrl ?? undefined} target="_blank" rel="noreferrer">{item.sourceUrl}</a>{item.capturedUnixMs ? <small>{text("获取于", "Captured")} {formatModifiedDate(item.capturedUnixMs, locale)} · {item.freshUntilUnixMs && item.freshUntilUnixMs >= Date.now() ? text("有效", "Current") : text("需更新", "Refresh needed")}</small> : null}</details> : null}{item.verification === "file" && item.materialKind && item.status !== "not_applicable" ? <><small className="accepted-material-formats">{text("支持：", "Accepted: ")}{materialAcceptedFormats(item.materialKind, locale, item.declaration?.allowedExtensions)}</small><GuidedButton className="secondary-button checklist-add-file" type="button" disabled={busy} prerequisite={() => !(checkTargetReady?.() ?? targetReady) && journalPrerequisite(text("需要先到“目标期刊”确认当前版本的主投期刊，并获取有效官方要求。", "Confirm a primary journal for this version and capture current official requirements in Target Journal first."), text("前往目标期刊", "Go to Target Journal"))} aria-label={text(`为${copy.label}添加文件`, `Add file for ${copy.label}`)} onClick={() => onAdd(item.materialKind!, item.id)}>{busy ? text("正在添加…", "Adding…") : text("为此要求添加文件", "Add file for this requirement")}</GuidedButton></> : null}{item.confirmable ? <RequirementDecision key={`${item.id}-${item.decision}-${item.decisionReason}`} workspaceId={workspace.id} item={item} onUpdated={onCatalogUpdated} disabled={busy || confirmingRequirementId !== null} /> : null}</div><span>{item.blocking ? text("必需", "Required") : text("建议", "Recommended")}</span></li>; })}</ul></section>; })}</div></section> : null}
    {activeView === "files" ? <section id="material-panel-files" role="tabpanel" aria-labelledby="material-view-files" className="material-view-panel">{catalog && catalog.materials.length > 0 ? <section className="material-file-list"><h3>{text("已保存的附加文件", "Stored supporting files")}</h3><ul>{catalog.materials.map((material) => {
      const current = material.manuscriptVersion === workspace.snapshotVersion && (!catalog.declarationPlan || (material.targetSelectionId === catalog.declarationPlan.targetSelectionId && material.requirementSnapshotId === catalog.declarationPlan.requirementSnapshotId));
      const boundItem = catalog.checklist.find((item) => item.id === material.checklistItemId);
      const eligible = current && Boolean(boundItem) && boundItem?.status !== "later" && boundItem?.status !== "not_applicable";
      const confirmingDelete = deleteMaterialId === material.materialId;
      return <li key={material.materialId} data-current={current}>
        <Icon name="file" />
        <div><strong>{material.originalName}</strong><span>{formatBytes(material.sizeBytes, locale)} · {material.contentHash.slice(0, 10)} · {current ? text("当前版本", "Current version") : text("历史材料", "Historical material")}</span>{!eligible ? <small>{boundItem?.status === "later" ? text("后续阶段材料，当前投稿包不纳入", "Later-stage material; excluded from the current package") : boundItem?.status === "not_applicable" ? text("当前论文不适用，未纳入投稿包", "Not applicable to this manuscript; excluded from the package") : text("需关联到当前版本和当前要求后才能组包", "Link to the current version and requirement before packaging")}</small> : null}{material.validationIssues.map((issue) => <small key={issue}>{issue}</small>)}</div>
        {!current && targetReady ? <HistoricalMaterialReuse workspaceId={workspace.id} material={material} items={catalog.checklist} onUpdated={onCatalogUpdated} /> : null}
        <div className="material-file-actions">
          {!eligible ? <GuidedButton className="text-button" disabled={busy} prerequisite={() => !(checkTargetReady?.() ?? targetReady) ? journalPrerequisite(text("请先核验当前版本投稿目标和官方要求。", "Verify the current version’s target and official requirements first."), text("前往目标期刊", "Go to Target Journal")) : !current ? { message: text("这是历史材料，请核对后关联到当前上传项。", "This is historical material. Review and link it to a current upload requirement."), scope: "li[data-current]", target: ".historical-material-reuse select" } : { message: text("当前要求不允许纳入此文件，请先核对适用性与投稿阶段。", "The current requirement does not allow this file. Review applicability and submission stage first."), prepare: () => setActiveView("requirements"), target: "#material-panel-requirements" }}>{text("查看纳入条件", "Review inclusion requirements")}</GuidedButton> : null}
          <label className="material-include-control"><input type="checkbox" checked={material.included && eligible} disabled={busy || !eligible} title={!eligible ? text("文件需匹配当前版本、目标与适用要求，才能纳入；请先核对要求或复用历史文件。", "The file must match the current version, target and applicable requirement. Review requirements or reuse the historical file first.") : undefined} onChange={(event) => onSetIncluded(material.materialId, event.target.checked)} /><span>{text("纳入组包", "Include")}</span></label>
    ...
    <PaneAction
      ...
      title={!targetReady ? text("先核验目标期刊", "Verify the target journal first") : authorInputsReady ? (catalog?.targetCheckReady ? text("查看当前目标检查", "Review the current target check") : text("进入目标检查", "Continue to target checks")) : text("可先检查，再补齐必需项", "Check now, then complete required items")}
      ...
      buttonLabel={!targetReady ? text("返回目标期刊", "Return to target journal") : authorInputsReady ? (catalog?.targetCheckReady ? text("查看检查结果", "Review check results") : text("继续检查当前稿件", "Continue to manuscript checks")) : text("运行本地基础检查", "Run local basic checks")}
      disabled={busy}
      prerequisite={targetReady ? () => !(checkTargetReady?.() ?? targetReady) && journalPrerequisite(text("请到目标期刊确认当前投稿目标与有效官方要求。", "Confirm the current target and official requirements in Target Journal."), text("前往目标期刊", "Go to Target Journal")) : null}
      onClick={onContinue}
    />
  </>;
}
...
export function TargetPackageAssembly({ plan, busy, onSetIncluded }: { plan: TargetSubmissionPackagePlan | null; busy: boolean; onSetIncluded: (materialId: string, included: boolean) => void }) {
  const { locale, text } = useI18n();
  ...
  const includedCount = new Set(plan.files.filter((file) => file.included).map((file) => file.relativePath)).size;
  return <section className="package-assembly" aria-labelledby="package-assembly-heading">
    ...
    <ol className="package-file-plan">{plan.files.map((file) => <li key={file.materialId ?? file.relativePath} data-included={file.included}><Icon name={file.included ? "check" : "close"} /><div><div><strong>{file.displayName}</strong><span>{packageRoleLabel(file.role, locale)} · {file.required ? text("必需", "Required") : text("可选", "Optional")}</span></div><code>{file.relativePath}</code><small>{file.checklistLabel ? text(`对应上传项：${file.checklistLabel}`, `Upload slot: ${file.checklistLabelEn ?? file.checklistLabel}`) : text("系统主稿", "System manuscript") } · SHA-256 {file.contentHash.slice(0, 12)}</small>{file.validationIssues.map((issue) => <p key={issue}>{issue}</p>)}</div>{file.materialId && file.inclusionAvailable === false ? <GuidedButton className="text-button" disabled={busy} prerequisite={{ message: text("此文件当前不可纳入，请到投稿资料核对适用性与投稿阶段。", "This file cannot be included now. Review applicability and submission stage in Materials."), destination: "materials", itemId: file.checklistItemId ?? undefined, actionLabel: text("前往投稿资料", "Go to Materials"), target: file.checklistItemId ? `[id=${JSON.stringify(`requirement-${file.checklistItemId}`)}]` : "#material-panel-requirements" }}>{text("查看纳入条件", "Review inclusion requirements")}</GuidedButton> : null}{file.materialId ? <label><input type="checkbox" checked={file.included} disabled={busy || file.inclusionAvailable === false} title={file.inclusionAvailable === false ? text("此文件不适用于当前投稿阶段，请在投稿资料中核对要求。", "This file does not apply to the current submission stage. Review its requirement in Submission materials.") : undefined} onChange={(event) => onSetIncluded(file.materialId!, event.target.checked)} /><span>{text("纳入", "Include")}</span></label> : <span className="package-fixed-file">{text("固定", "Fixed")}</span>}</li>)}</ol>
    {plan.blockers.length > 0 ? <div className="package-preflight-messages" data-tone="blocked"><strong>{text("必须处理", "Must resolve")}</strong><ul>{plan.blockers.map((message) => <li key={message}>{message.startsWith("DECLARATION_CHECK:") ? localizeBackendText(locale, message) : message}</li>)}</ul></div> : null}
    {plan.warnings.length > 0 ? <details className="package-preflight-messages"><summary>{text(`${plan.warnings.length} 项上传前提示`, `${plan.warnings.length} pre-upload note(s)`)}</summary><ul>{plan.warnings.map((message) => <li key={message}>{message.startsWith("DECLARATION_CHECK:") ? localizeBackendText(locale, message) : message}</li>)}</ul></details> : null}
  </section>;
}

export function JournalMatchStage({ workspace, selectedTarget, targetPlan, requirementSnapshots, selectingTarget, targetPlanBusyId, requirementBusyId, onRecommendationGenerated, onSelectTarget, onClearPrimary, onAddBackup, onPromoteBackup, onDiscoverRequirements, onSaveManualRequirements, onContinue }: { workspace: WorkspaceSummary; selectedTarget: SubmissionTargetSelection | null; targetPlan: SubmissionTargetPlan | null; requirementSnapshots: JournalRequirementSnapshot[]; selectingTarget: boolean; targetPlanBusyId: string | null; requirementBusyId: string | null; onRecommendationGenerated: () => void; onSelectTarget: (recommendationRunId: string, journalId: string) => void; onClearPrimary: (selectionId: string) => void; onAddBackup: (recommendationRunId: string, journalId: string) => void; onPromoteBackup: (selectionId: string, reason: string) => void; onDiscoverRequirements: DiscoverOfficialSource<JournalRequirementSnapshot>; onSaveManualRequirements: (selectionId: string, sourceUrl: string, requirementText: string) => void; onContinue: () => void }) {
  const { locale, text } = useI18n();
  ...
  useEffect(() => { if (run) { resultsRef.current?.scrollIntoView?.({ block: "start" }); resultsRef.current?.focus(); } }, [run?.runId]);
  ...
  return <>
    <span hidden data-guidance-loading={historyBusy ? "true" : "false"} />
    ...
    <details className="advanced-settings"><summary>{text("学校、用途和准备期限（可选）", "Institution, purpose, and preparation date (optional)")}</summary>
    ...
    <section className="journal-preferences" aria-labelledby="journal-preferences-heading"><h3 id="journal-preferences-heading">{text("调整推荐条件", "Adjust recommendation conditions")}</h3><div>
      ...
      <label>{text("文章类型", "Article type")}<select id="journal-article-type" value={preferences.articleType} onChange={(event) => update("articleType", event.target.value as ArticleTypePreference)}><option value="auto">{text("自动识别", "Auto detect")}</option><option value="research">{text("研究论文", "Research")}</option><option value="review">{text("综述", "Review")}</option><option value="application">{text("应用型", "Application")}</option></select></label>
    ...
    </div><GuidedButton className="primary-button" type="button" id="generate-recommendations" data-guide-selection={!currentRunReady ? "true" : undefined} disabled={busy} prerequisite={!profileComplete ? { message: text("请将准备期限设为明天或之后。", "Set the preparation date to tomorrow or later."), target: "#journal-deadline" } : !institutionTextReady ? { message: text("学校要求原文至少需要 40 个字符；也可以清空后只生成基础推荐。", "Provide at least 40 characters of institution policy, or clear it to generate basic recommendations."), target: "#institution-policy-text" } : null} onClick={() => void calculate()}>{busy ? text("正在分析主稿并计算…", "Analyzing the manuscript…") : hasInstitutionText ? text("结合校规生成初步推荐", "Generate preliminary recommendations with institution rules") : text("根据当前论文生成初步推荐", "Generate preliminary recommendations from this manuscript")}<Icon name="arrow" /></GuidedButton>{!profileComplete ? <p className="journal-profile-hint">{text("请选择未来的投稿准备截止日期。", "Choose a future submission-preparation deadline.")}</p> : hasInstitutionText && institutionRequirementText.trim().length < 40 ? <p className="journal-profile-hint">{text("学校要求原文至少需要 40 个字符。", "Institution requirement text must contain at least 40 characters.")}</p> : null}</section>
    {error ? <p className="inline-warning"><Icon name="warning" />{error}</p> : null}
    {run ? <>
      <div ref={resultsRef} tabIndex={-1} role="status"><h3>{text("推荐已生成", "Recommendations generated")}</h3><p>{text("本地目录、主题、语言、文章类型和准备条件共同筛选；候选不足时不会填充低匹配期刊，推荐不代表录用概率。", "The local catalog, topic, language, article type, and preparation conditions filter this shortlist. Undersized groups are not filled with poor matches. Recommendations are not acceptance probabilities.")}</p><p>{text("本次推荐采用的文章类型：", "Article type used for this run: ")}{articleTypeLabel(run.resolvedArticleType, locale)} · {text("依据标题和摘要的类型线索；请核对后使用。", "Based on title and abstract cues; review before using.")}</p><button type="button" className="text-button" onClick={() => { const field = document.getElementById("journal-article-type"); field?.scrollIntoView?.({ block: "center" }); field?.focus(); if (field) highlightGuidanceTarget(field); }}>{text("修改文章类型", "Change article type")}</button></div>
      ...
      {selectedTarget && !selectedTargetCurrent ? <button className="secondary-button" type="button" onClick={() => { void invoke("reconfirm_previous_target", { workspaceId: workspace.id }).then(onRecommendationGenerated).catch(reason => setError(localizeBackendText(locale, String(reason)))); }}>{text("重新确认沿用上一版期刊与仍有效的官方依据", "Reconfirm the previous journal and still-current official evidence")}</button> : null}
      ...
      <div className="journal-region-sections"><JournalRegionPortfolio title={text("中国期刊与出版社", "China journals & publishers")} portfolio={run.domestic} locale={locale} runId={run.runId} workspaceVersion={workspace.snapshotVersion} recommendationCurrent={currentRunReady} selectionDisabled={!currentRunReady} selectedTarget={selectedTarget} backupJournalIds={backupJournalIds} selectingTarget={selectingTarget} targetPlanBusyId={targetPlanBusyId} onSelectTarget={onSelectTarget} onClearPrimary={onClearPrimary} onAddBackup={onAddBackup} /><JournalRegionPortfolio title={text("全球期刊与出版社", "Global journals & publishers")} portfolio={run.international} locale={locale} runId={run.runId} workspaceVersion={workspace.snapshotVersion} recommendationCurrent={currentRunReady} selectionDisabled={!currentRunReady} selectedTarget={selectedTarget} backupJournalIds={backupJournalIds} selectingTarget={selectingTarget} targetPlanBusyId={targetPlanBusyId} onSelectTarget={onSelectTarget} onClearPrimary={onClearPrimary} onAddBackup={onAddBackup} /></div>
      {selectedTargetCurrent && selectedTarget ? <details className="advanced-settings"><summary>{text("补充期刊画像（可选，不影响基础检查）", "Supplement journal profile (optional; basic checks remain available)")}</summary><JournalProfileDiscoveryPanel target={selectedTarget} record={currentProfileDiscovery} busy={profileDiscoveryBusy} onDiscover={discoverSelectedJournalProfile} /></details> : null}
...
export function JournalRequirementCapture({ target, snapshot, busy, onDiscover, onSaveManual, compact = false }: { target: SubmissionTargetSelection; snapshot: JournalRequirementSnapshot | null; busy: boolean; onDiscover: DiscoverOfficialSource<JournalRequirementSnapshot>; onSaveManual: (selectionId: string, sourceUrl: string, requirementText: string) => void; compact?: boolean }) {
  ...
  return <section className="journal-requirement-capture" data-ready={ready}>
    ...
    {snapshot?.limitations.filter((limitation) => limitation !== "OFFICIAL_HTTP_EVIDENCE").map((limitation) => <small key={limitation}>{localizeBackendText(locale, limitation)}</small>)}
    <details className="manual-requirement-input" open={!automaticFetchAvailable || snapshot?.status === "requires_manual_review"}><summary>{automaticFetchAvailable ? text("网页无法读取？粘贴官方原文", "Page unavailable? Paste official text") : text("粘贴官方投稿要求，继续准备", "Paste official requirements to continue")}</summary><label>{text("官方来源网址", "Official source URL")}<input type="url" inputMode="url" value={manualUrl} onChange={(event) => setManualUrl(event.target.value)} /></label><label>{text("作者指南原文", "Author-guide text")}<textarea value={manualText} maxLength={100000} onChange={(event) => setManualText(event.target.value)} placeholder={text("粘贴与格式、匿名审稿、图表、声明、费用等有关的官方原文……", "Paste official text covering format, anonymous review, figures, declarations, fees, and related requirements…")} /></label><label className="confirmation-control"><input type="checkbox" checked={manualConfirmed} onChange={(event) => setManualConfirmed(event.target.checked)} /><span>{text("我确认这段原文来自该期刊或出版社的官方作者指南", "I confirm this text is from the journal or publisher's official author guide")}</span></label><GuidedButton className="text-button" type="button" disabled={busy} prerequisite={!manualSourceAllowed ? { message: text("请填写有效的官方 HTTP 或 HTTPS 来源网址。", "Enter a valid official HTTP or HTTPS source URL."), target: 'input[type="url"]', scope: ".manual-requirement-input" } : manualText.trim().length < 20 ? { message: text("请粘贴至少 20 个字符的官方作者指南原文。", "Paste at least 20 characters of official author-guide text."), target: "textarea", scope: ".manual-requirement-input" } : !manualConfirmed ? { message: text("请先确认原文来自期刊或出版社官方指南。", "Confirm the text comes from the journal or publisher’s official guide."), target: 'input[type="checkbox"]', scope: ".manual-requirement-input" } : null} onClick={submitManual}>{text("保存并生成本地快照", "Save local snapshot")}</GuidedButton></details>
  </section>;
}
...
function JournalRegionPortfolio({ title, portfolio, locale, runId, workspaceVersion, recommendationCurrent, selectionDisabled, selectedTarget, backupJournalIds, selectingTarget, targetPlanBusyId, onSelectTarget, onClearPrimary, onAddBackup }: { title: string; portfolio: JournalRecommendationPortfolio; locale: Locale; runId: string; workspaceVersion: number; recommendationCurrent: boolean; selectionDisabled: boolean; selectedTarget: SubmissionTargetSelection | null; backupJournalIds: Set<string>; selectingTarget: boolean; targetPlanBusyId: string | null; onSelectTarget: (runId: string, journalId: string) => void; onClearPrimary: (selectionId: string) => void; onAddBackup: (runId: string, journalId: string) => void }) {
  const { text } = useI18n();
  ...
  const shared = { locale, runId, workspaceVersion, recommendationCurrent, selectionDisabled, selectedTarget, backupJournalIds, selectingTarget, targetPlanBusyId, focusedJournalId, onHighlight: setFocusedJournalId, onSelectTarget, onClearPrimary, onAddBackup };
...
function JournalRecommendationList({ tierCode, title, description, items, locale, runId, workspaceVersion, recommendationCurrent, selectionDisabled, selectedTarget, backupJournalIds, selectingTarget, targetPlanBusyId, focusedJournalId, onHighlight, onSelectTarget, onClearPrimary, onAddBackup }: { tierCode: "R" | "M" | "S"; title: string; description: string; items: JournalRecommendation[]; locale: Locale; runId: string; workspaceVersion: number; recommendationCurrent: boolean; selectionDisabled: boolean; selectedTarget: SubmissionTargetSelection | null; backupJournalIds: Set<string>; selectingTarget: boolean; targetPlanBusyId: string | null; focusedJournalId: string | null; onHighlight: (journalId: string) => void; onSelectTarget: (runId: string, journalId: string) => void; onClearPrimary: (selectionId: string) => void; onAddBackup: (runId: string, journalId: string) => void }) {
  const { text } = useI18n();
...
function StructureCheckSummary({ report }: { report: StructureReport }) {
  const { locale, text } = useI18n();
  return <section className="check-structure-summary"><header><div><span>{text("结构提取完成", "Structure extracted")}</span><h3>{report.title ?? text("未检测到标题", "No title detected")}</h3></div><strong>v{report.sourceSnapshotVersion}</strong></header><div className="metric-row"><Metric label={text("作者", "Authors")} value={report.authors.length} /><Metric label={text("章节", "Sections")} value={report.sections.length} /><Metric label={text("图", "Figures")} value={report.figureCount} /><Metric label={text("表", "Tables")} value={report.tableCount} /></div>{report.warnings.length > 0 ? <details className="structure-warnings"><summary>{text(`有 ${report.warnings.length} 项提取提示，请核对原稿`, `${report.warnings.length} extraction notes: review against the source`)}</summary>{report.warnings.map((warning) => <p className="inline-warning" key={warning}><Icon name="warning" />{localizeBackendText(locale, warning)}</p>)}</details> : null}</section>;
}
  ...
  return <>
    ...
    <details className="version-revision-option" open={candidate !== null}>
      ...
      <section className="version-save-card" aria-labelledby="version-save-heading">
        ...
        {candidate ? <GuidedButton className="secondary-button version-primary" type="button" onClick={onSave} disabled={selecting || saving} prerequisite={!formatMatches && { message: text("修改稿需与当前稿件保持相同格式，请重新选择文件。", "Choose a revision in the same format as the current manuscript."), target: ".version-save-heading button" }}>{saving ? text("正在保存并复查…", "Saving and rechecking…") : text(`保存为 v${currentVersion + 1} 并复查`, `Save as v${currentVersion + 1} and recheck`)}<Icon name="check" /></GuidedButton> : null}
      </section>
    </details>
  </>;
}
...
function TargetRuleSelector({ ruleCatalog, selectedRulePackIds, loading, structureReady, onToggle, onContinue, actionLabel, disabled = false }: { ruleCatalog: RulePackCatalogItem[]; selectedRulePackIds: string[]; loading: boolean; structureReady: boolean; onToggle: (rulePackId: string) => void; onContinue: () => void; actionLabel?: string; disabled?: boolean }) {
  ...
  return <>
    ...
    <details className="advanced-settings"><summary>{text("选择增强规则（可选，查看全部）", "Choose enhanced rules (optional, view all)")}</summary><div className="rule-catalog" aria-label={text("可选出版标准", "Optional publication standards")}>
...
function SubmissionElementsDesk({ catalog, draft, values, result, loading, saving, selectedPublisherCount, onValueChange, onSave, onContinue }: { catalog: SubmissionElementCatalog | null; draft: RevisionDraft | null; values: Record<string, string>; result: RevisionSet | null; loading: boolean; saving: boolean; selectedPublisherCount: number; onValueChange: (field: string, value: string) => void; onSave: () => void; onContinue: () => void }) {
  ...
  return <>
    ...
    <PaneAction label={changedCount > 0 ? text(`${changedCount} 项待保存`, `${changedCount} change(s) pending`) : text("下一步", "Next")} title={changedCount > 0 ? text(`保存为新版本 v${(draft?.baseVersion ?? 0) + 1}`, `Save as new version v${(draft?.baseVersion ?? 0) + 1}`) : text("查看当前投稿包预检", "Review the current package precheck")} copy={changedCount > 0 ? text("保存后自动重提取和复查；新版本需要重新确认期刊目标与官方要求，原稿与历史不会被覆盖。", "Saving automatically extracts and rechecks. The new version must reconfirm its journal target and official requirements; the source and history remain unchanged.") : text("没有待保存修改；版本历史仍可从高级记录单独查看。", "There are no unsaved changes; version history remains available separately under advanced records.")} buttonLabel={changedCount > 0 ? (saving ? text("保存并复查中…", "Saving and rechecking…") : text("保存新版本并重新确认目标", "Save and reconfirm target")) : text("进入投稿包", "Continue to package")} disabled={saving} onClick={changedCount > 0 ? onSave : onContinue} />
  </>;
}
...
function EvidencePane({ stage, workspace, structureReport, readinessReport, knowledgeBodySnapshot = null, knowledgeBodyRecord = null, attestation = null, submission = null, submissionExport = null, submissionMaterials = null, submissionTargetSelection = null, submissionTargetPlan = null, journalRequirementSnapshots = [], ruleCatalog = [], selectedRulePackIds = [], submissionElementCatalog = null, revisionDraft = null, revisionValues = {}, revisionResult = null, versionHistory = null, selectedVersion = null, versionComparison = null, isComparingVersions = false }: PaneProps) {
  ...
  if (stage === "submission") return <EvidenceFrame kicker={text("投稿证据", "Submission evidence")} title={submission ? text("已登记投稿", "Submission recorded") : text("出版社投稿包", "Publisher submission package")}><PackageExportSummary status={submissionMaterials?.exportStatus} ready={submissionMaterials?.targetCheckReady} />{submission ? <LifecycleEvidence id={submission.submissionId} hash={submission.recordHash} items={[[text("目标", "Target"), submission.target], [text("回执", "Receipt"), submission.receipt ?? text("未填写", "Not provided")], [text("自动存证", "Automatic attestation"), submission.attestationId]]} /> : <div className="package-preview"><span>MANUSCRIPTDOCK</span><h2>{submissionTargetSelection ? (locale === "en" ? submissionTargetSelection.nameEn : submissionTargetSelection.name) : text("尚未选择目标", "No target selected")}</h2><p>{text("作者控制的投稿资料交付", "Author-controlled publisher handoff")}</p><dl><div><dt>{text("稿件版本", "Manuscript version")}</dt><dd>v{workspace.snapshotVersion}</dd></div><div><dt>{text("附加资料", "Supporting files")}</dt><dd>{submissionMaterials?.materials.length ?? 0}</dd></div><div><dt>{text("导出状态", "Export status")}</dt><dd>{packageExportLabel(submissionMaterials?.exportStatus, submissionMaterials?.targetCheckReady, locale)}</dd></div></dl><small>{text("submission 供出版社；records 仅供本地留档。导出不等于已投稿。", "submission is publisher-facing; records is local-only. Export does not mean submitted.")}</small></div>}</EvidenceFrame>;
...
function PaneAction({ label, title, copy, buttonLabel, disabled = false, prerequisite, onClick }: { label: string; title: string; copy: string; buttonLabel: string; disabled?: boolean; prerequisite?: PrerequisiteCheck; onClick: () => void }) { return <div className="pane-action"><div><span>{label}</span><h3>{title}</h3><p>{copy}</p></div><GuidedButton className="primary-button" type="button" disabled={disabled} prerequisite={prerequisite} onClick={onClick}>{buttonLabel}<Icon name="arrow" /></GuidedButton></div>; }
...
function conciseKnowledgeText(value: string, limit = 128) {
  ...
  const punctuationBoundary = visibleCharacters.reduce((last, character, index) => punctuation.has(character) ? index + 1 : last, 0);
  const wordBoundary = visibleCharacters.reduce((last, character, index) => character === " " ? index : last, -1);
  const boundary = punctuationBoundary >= minimumBoundary ? punctuationBoundary : wordBoundary >= minimumBoundary ? wordBoundary : limit;
  return `${visibleCharacters.slice(0, boundary).join("").trimEnd()}…`;
}
...
function KnowledgeSpatialMap({ workspace, knowledgeBodySnapshot = null }: { workspace: WorkspaceSummary; knowledgeBodySnapshot?: AcademicKnowledgeBodySnapshot | null }) {
  ...
  const semanticElements: Array<{ key: SemanticElementKind; label: string; element?: ExtractedKnowledgeElement }> = [
    { key: "claim", label: text("Claim · 核心命题", "Claim · Central proposition"), element: extraction?.claim },
    { key: "scope", label: text("Scope · 适用边界", "Scope · Boundary"), element: extraction?.scope },
    { key: "method", label: text("Method · 方法", "Method"), element: extraction?.method },
    { key: "result", label: text("Result · 结果", "Result"), element: extraction?.result },
    { key: "evidence", label: text("Evidence · 证据", "Evidence"), element: extraction?.evidence },
  ];
  const summaries = semanticElements.map((item) => {
    ...
    const pendingSummary = item.key === "claim"
      ? text("未识别到可靠核心命题，请作者确认", "No reliable central proposition was identified; author review is required")
      : text("当前分解未形成可靠内容", "No reliable content was formed by the current decomposition");
    return { ...item, candidate, summary: candidate ? conciseKnowledgeText(candidate.text) : pendingSummary };
  });
    ...
    <section className="knowledge-space" aria-label={text("动态知识点云与关联网络", "Dynamic knowledge point cloud and relationship network")}>
      <div className="knowledge-view-switch" role="tablist" aria-label={text("知识体网络层级", "Knowledge-network level")}>
        {(["single", "pair", "network"] as KnowledgeView[]).map((item, index) => {
          ...
          return <GuidedButton key={item} type="button" role="tab" aria-selected={view === item} prerequisite={!enabled && { message: text(`此视图至少需要 ${item === "pair" ? 2 : 3} 个知识体；当前只有 ${bodyCount} 个。`, `This view requires at least ${item === "pair" ? 2 : 3} knowledge bodies; ${bodyCount} currently available.`), target: ".knowledge-view-switch", scope: ".knowledge-space" }} title={enabled ? labels[index] : text("点击查看需要的知识体数量", "Click to see how many knowledge bodies are needed")} onClick={() => setView(item)}>{labels[index]}</GuidedButton>;
        })}
      </div>
      {view === "single" ? (
      ...
      {view === "single" ? <section className="knowledge-summary-legend" aria-labelledby="knowledge-summary-heading"><header><div><span>{text("Claim 是核心命题，不等同于摘要", "A Claim is the central proposition, not an abstract")}</span><h3 id="knowledge-summary-heading">{text("知识命题与来源", "Knowledge propositions and sources")}</h3></div><strong>{text("逐项可追溯", "Traceable by item")}</strong></header><ol>{summaries.map((item) => <li key={item.key} data-state={item.element?.state ?? "pending"}><div className="knowledge-summary-label"><strong>{item.label}</strong><span>{item.element?.state === "established" ? text("作者已确认", "Confirmed") : item.candidate ? text("提取候选", "Candidate") : text("未建立", "Pending")}</span></div><p>{item.summary}</p>{item.candidate ? <small>{item.candidate.sourceLabel}{item.candidate.sourceFragmentId ? ` · ${item.candidate.sourceFragmentId}` : ""} · {item.candidate.confidencePercent}%</small> : null}</li>)}</ol></section> : null}
...
export function KnowledgeDialoguePanel({ workspace, knowledgeBodyRecord, completionPrerequisite }: { workspace: WorkspaceSummary; knowledgeBodyRecord: KnowledgeBodyRecord | null; completionPrerequisite?: Prerequisite }) {
  const { locale, text } = useI18n();
  ...
  const [question, setQuestion] = useGuidanceDraft(`knowledge-question-${workspace.id}`);
  ...
  return <section className="knowledge-dialogue" aria-labelledby="knowledge-dialogue-title">
    ...
    {settingsOpen ? <section className="model-settings" aria-labelledby="model-settings-title">
      ...
      <div className="model-slot-grid">{slotDrafts.map((slot) => <fieldset className="model-slot" data-model-slot={slot.role} key={slot.role}>
        <legend>{modelSlotLabel(slot.role, locale)}</legend>
        ...
        <label className="model-enabled"><input type="checkbox" checked={slot.enabled} onChange={(event) => updateSlot(slot.role, "enabled", event.target.checked)} />{text("启用此槽位", "Enable this slot")}</label>
        <label>{text("提供方名称", "Provider label")}<input data-model-field="providerLabel" value={slot.providerLabel} onChange={(event) => updateSlot(slot.role, "providerLabel", event.target.value)} placeholder={text("例如：OpenAI", "e.g. OpenAI")} /></label>
        <label>{text("API 地址", "API base URL")}<input aria-label={text("API 地址", "API base URL")} data-model-field="baseUrl" value={slot.baseUrl} aria-invalid={isDeepSeekDocumentationUrl(slot.baseUrl)} onChange={(event) => updateSlot(slot.role, "baseUrl", event.target.value)} placeholder="https://api.example.com/v1" inputMode="url" />{isDeepSeekDocumentationUrl(slot.baseUrl) ? <span className="model-field-error" role="alert">{text("这是 DeepSeek 文档页，不是 API。请改用 https://api.deepseek.com", "This is the DeepSeek documentation site, not its API. Use https://api.deepseek.com")}</span> : null}</label>
        <label>{text("模型名称", "Model name")}<input data-model-field="model" value={slot.model} onChange={(event) => updateSlot(slot.role, "model", event.target.value)} placeholder="model-id" /></label>
        <label>{text("API Key", "API key")}<input aria-label={text("API Key", "API key")} type="password" autoComplete="new-password" data-model-field="apiKey" value={slot.apiKey} aria-invalid={slot.enabled && !slot.hasApiKey && !slot.apiKey.trim()} onChange={(event) => { updateSlot(slot.role, "apiKey", event.target.value); updateSlot(slot.role, "clearApiKey", false); }} placeholder={slot.hasApiKey ? text("已安全保存；留空表示保留", "Stored securely; leave blank to retain") : text("输入后保存到系统凭据库", "Saved to the system credential store")} />{slot.enabled && !slot.hasApiKey && !slot.apiKey.trim() ? <span className="model-field-error" role="alert">{text("启用模型需要 API Key；输入后点击“保存模型设置”。", "An API key is required. Enter it, then save the model settings.")}</span> : null}</label>
        <label className="model-clear-key"><input type="checkbox" checked={slot.clearApiKey} onChange={(event) => updateSlot(slot.role, "clearApiKey", event.target.checked)} disabled={!slot.hasApiKey} />{text("删除已保存的 Key", "Delete saved key")}{!slot.hasApiKey && <small>{text("尚无已保存密钥，无需清除", "No saved key to delete")}</small>}</label>
      </fieldset>)}</div>
      <div className="model-settings-actions"><p>{text("应用界面不会读取或回显明文 Key。同一次应用运行中，每个已保存 Key 只向系统凭据库读取一次；作者点击“询问知识体”即授权该次调用。DeepSeek V4 问答会关闭思考模式，把输出额度保留给最终回答。", "The interface never reads or reveals plaintext keys. During one app session, each stored key is read from the credential store only once; clicking “Ask knowledge body” authorizes that call. DeepSeek V4 requests disable thinking mode so the output allowance remains available for the final answer.")}</p><GuidedButton className="primary-button" type="button" disabled={isSavingSettings} prerequisite={invalidEnabledDraft && modelSettingsPrerequisite(slotDrafts, locale)} onClick={() => void saveSettings()}>{isSavingSettings ? text("保存中…", "Saving…") : text("保存模型设置", "Save model settings")}</GuidedButton></div>
    </section> : null}
    ...
    {activeTab === "owner" ? <div className="dialogue-owner">
      ...
      <form className="knowledge-composer" onSubmit={(event) => { event.preventDefault(); void ask(); }}>
        ...
        <label className="composer-question"><span>{text("问题或需求", "Question or request")}</span><textarea rows={3} maxLength={4000} id="knowledge-question" value={question} onChange={(event) => setQuestion(event.target.value)} placeholder={text("例如：这个 Claim 目前缺少哪些来源锚点？", "For example: Which source anchors are still missing for this Claim?")} disabled={isAsking} /></label>
        <div className="composer-submit"><p>{text("本次只发送脱敏后的知识体投影与问题；作者姓名、联系方式、身份标识、源文件和本机路径均不发送，回答不会自动改写知识体。", "Only the redacted knowledge-body projection and question are sent. Author names, contact details, identifiers, source files, and local paths are excluded; answers cannot automatically modify the knowledge body.")}</p><GuidedButton className="primary-button" type={configuredSlots.length === 0 ? "button" : "submit"} disabled={isAsking} prerequisite={!knowledgeBodyRecord ? completionPrerequisite : configuredSlots.length > 0 && !question.trim() ? { message: text("请先填写想询问的问题。", "Enter a question first."), target: "#knowledge-question" } : null} onClick={configuredSlots.length === 0 && knowledgeBodyRecord ? () => setSettingsOpen(true) : undefined}>{isAsking ? text("模型回答中…", "Model is answering…") : !knowledgeBodyRecord ? text("先固化知识体", "Finalize knowledge body first") : configuredSlots.length === 0 ? text("打开模型设置", "Open model settings") : text("询问知识体", "Ask knowledge body")}</GuidedButton></div>
