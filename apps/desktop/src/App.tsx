...
function ManuscriptDockApp() {
  ...
  async function addSubmissionMaterials(kind: SubmissionMaterialKind, checklistItemId?: string) {
    ...
    setErrorMessage(null);
    try {
      const catalog = await invoke<SubmissionMaterialCatalog | null>("add_submission_materials", { workspaceId: activeWorkspace.id, kind, checklistItemId: checklistItemId ?? null });
      if (catalog) {
...
function SubmissionMaterialsCenter({ workspace, catalog, busy, confirmingRequirementId, targetReady, onAdd, onSetIncluded, onConfirm, onContinue }: { workspace: WorkspaceSummary; catalog: SubmissionMaterialCatalog | null; busy: boolean; confirmingRequirementId: string | null; targetReady: boolean; onAdd: (kind: SubmissionMaterialKind, checklistItemId?: string) => void; onSetIncluded: (materialId: string, included: boolean) => void; onConfirm: (itemId: string, confirmed: boolean) => void; onContinue: () => void }) {
  const { locale, text } = useI18n();
  ...
  return <>
    ...
    <section className="materials-checklist" aria-labelledby="materials-checklist-heading"><header><div><span>{text("动态投稿清单", "Dynamic submission checklist")}</span><h3 id="materials-checklist-heading">{authorInputsReady ? text("作者侧必需资料与确认已完成", "Author-supplied files and confirmations are complete") : text("仍有必需项待处理", "Required items still need attention")}</h3></div><strong>{catalog ? `${catalog.requiredCompleted ?? 0}/${catalog.requiredTotal ?? 0}` : `v${workspace.snapshotVersion}`}</strong></header><div className="checklist-groups">{groups.map((group) => { const items = (catalog?.checklist ?? []).filter((item) => item.group === group); if (items.length === 0) return null; return <section className="checklist-group" key={group} aria-labelledby={`checklist-group-${group}`}><header><h4 id={`checklist-group-${group}`}>{localize(locale, ...groupLabels[group])}</h4><span>{items.length}</span></header><ul>{items.map((item) => { const copy = submissionChecklistCopy(item, locale); const confirmed = item.confirmable && item.status === "passed"; return <li key={item.id} data-status={item.status}><Icon name={item.status === "passed" ? "check" : "warning"} /><div><div className="checklist-item-heading"><strong>{copy.label}</strong><span>{submissionChecklistStatusLabel(item.status, locale)}</span></div><p>{copy.detail}</p>{item.requiredCount > 0 ? <small className="material-slot-count">{text(`已绑定 ${item.matchedMaterialIds.length}/${item.requiredCount} 个文件`, `${item.matchedMaterialIds.length}/${item.requiredCount} file(s) bound`)}</small> : null}{item.evidenceExcerpt ? <details className="checklist-evidence"><summary>{text("查看官方依据", "View official evidence")}</summary><blockquote>{item.evidenceExcerpt}</blockquote><a href={item.sourceUrl ?? undefined} target="_blank" rel="noreferrer">{item.sourceUrl}</a>{item.capturedUnixMs ? <small>{text("获取于", "Captured")} {formatModifiedDate(item.capturedUnixMs, locale)} · {item.freshUntilUnixMs && item.freshUntilUnixMs >= Date.now() ? text("有效", "Current") : text("需更新", "Refresh needed")}</small> : null}</details> : null}{item.verification === "file" && item.materialKind ? <button className="secondary-button checklist-add-file" type="button" disabled={busy || !targetReady} onClick={() => onAdd(item.materialKind!, item.id)}>{busy ? text("正在添加…", "Adding…") : text("为此要求添加文件", "Add file for this requirement")}</button> : null}{item.confirmable ? <button className="text-button checklist-confirm" type="button" disabled={confirmingRequirementId !== null} onClick={() => onConfirm(item.id, !confirmed)}>{confirmingRequirementId === item.id ? text("正在保存…", "Saving…") : confirmed ? text("撤销确认", "Revoke confirmation") : item.verification === "author" ? text("我已确认内容真实且适用", "I confirm this is accurate and applicable") : text("我已对照原文人工核验", "I verified this against the source")}</button> : null}</div><span>{item.blocking ? text("必需", "Required") : text("建议", "Recommended")}</span></li>; })}</ul></section>; })}</div></section>
    {catalog && catalog.materials.length > 0 ? <section className="material-file-list"><h3>{text("已保存的附加文件", "Stored supporting files")}</h3><ul>{catalog.materials.map((material) => { const current = material.manuscriptVersion === workspace.snapshotVersion; return <li key={material.materialId} data-current={current}><Icon name="file" /><div><strong>{material.originalName}</strong><span>{formatBytes(material.sizeBytes, locale)} · {material.contentHash.slice(0, 10)} · {current ? text("当前版本", "Current version") : text("历史材料", "Historical material")}</span>{material.validationIssues.map((issue) => <small key={issue}>{issue}</small>)}</div><label className="material-include-control"><input type="checkbox" checked={material.included} disabled={busy || !current} onChange={(event) => onSetIncluded(material.materialId, event.target.checked)} /><span>{text("纳入组包", "Include")}</span></label></li>; })}</ul></section> : null}
