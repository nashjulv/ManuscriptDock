...
export function SubmissionMaterialsCenter({ workspace, catalog, busy, confirmingRequirementId, targetReady, onAdd, onSetIncluded, onDelete, onConfirm, onCatalogUpdated, onContinue }: { onCatalogUpdated: (catalog: SubmissionMaterialCatalog) => void; workspace: WorkspaceSummary; catalog: SubmissionMaterialCatalog | null; busy: boolean; confirmingRequirementId: string | null; targetReady: boolean; onAdd: (kind: SubmissionMaterialKind, checklistItemId?: string) => void; onSetIncluded: (materialId: string, included: boolean) => void; onDelete: (materialId: string) => void; onConfirm: (itemId: string, confirmed: boolean) => void; onContinue: () => void }) {
  const { locale, text } = useI18n();
    ...
    {activeView === "files" ? <section id="material-panel-files" role="tabpanel" aria-labelledby="material-view-files" className="material-view-panel">{catalog && catalog.materials.length > 0 ? <section className="material-file-list"><h3>{text("已保存的附加文件", "Stored supporting files")}</h3><ul>{catalog.materials.map((material) => {
      ...
      return <li key={material.materialId} data-current={current}>
        ...
        <div className="material-file-actions">
          <label className="material-include-control"><input type="checkbox" checked={material.included && eligible} disabled={busy || !eligible} onChange={(event) => onSetIncluded(material.materialId, event.target.checked)} /><span>{text("纳入组包", "Include")}</span></label>
